...
function MultiHorizonForecastTable({ items = [], horizons = [] }) {
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.symbol} className={item.shadow_target ? 'temporal-shadow-target' : ''}>
              ...
              <td>{number(item.opportunity_gate_score, 4)} / {number(item.entry_threshold, 4)}</td>
...
export function TemporalIntelligencePanel({ capabilities = {}, onSessionExpired }) {
  ...
  const multiHorizonPolicy = result?.experiment === 'temporal_decision_intelligence_v4_multi_horizon'
  const decisionExperiment = ['temporal_decision_intelligence_v1', 'temporal_decision_intelligence_v2', 'temporal_decision_intelligence_v3', 'temporal_decision_intelligence_v4_multi_horizon'].includes(result?.experiment)
  const capitalPolicyV2 = ['temporal_decision_intelligence_v2', 'temporal_decision_intelligence_v3', 'temporal_decision_intelligence_v4_multi_horizon'].includes(result?.experiment)
  const capitalPolicyV3 = ['temporal_decision_intelligence_v3', 'temporal_decision_intelligence_v4_multi_horizon'].includes(result?.experiment)
  useEffect(() => {
      ...
      {result ? <>
        ...
        {multiHorizonPolicy && multiHorizonMetrics ? <>
          <section className="temporal-section">
            <div className="temporal-section-heading"><h3>{tr('Multi-Horizon Decision Engine')}</h3><span>{tr('Single BUY / HOLD / SELL / CASH policy')}</span></div>
            ...
            <MultiHorizonForecastTable items={multiHorizonForecasts} horizons={result.horizons || []} />
          </section>
        </> : null}
