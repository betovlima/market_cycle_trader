...
export function AdministrationPage({ onSessionExpired }: AppRecord) {
  ...
  const [extendDurations, setExtendDurations] = useState<AppRecord>({})
  const [sessionLimits, setSessionLimits] = useState<AppRecord>({})
  const [generatedAccess, setGeneratedAccess] = useState<AppRecord | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setSessionLimits((current: any) => {
        const next = { ...current }
        items.forEach((item: AppRecord) => {
  ...
  async function createInvitation(event: any) {
  ...
  async function regenerateAccessLink(invitation: AppRecord) {
    ...
    try {
      ...
      setExtendDurations((current: any) => ({
        ...current,
  ...
  async function updateInvitation(invitation: AppRecord, body: AppRecord, message: string, actionName: string) {
  ...
  async function extendInvitation(invitation: AppRecord) {
    const selectedDuration = extendDurations[invitation.id] ?? DEFAULT_DURATION_SECONDS
    ...
    setExtendDurations((current: any) => ({ ...current, [invitation.id]: DEFAULT_DURATION_SECONDS }))
  }

  async function saveSessionLimit(invitation: AppRecord) {
  ...
  async function deleteInvitation(invitation: AppRecord) {
  ...
  const counts = useMemo(() => ({
    pending: invitations.filter((item: AppRecord) => item.status === 'pending_verification').length,
    active: invitations.filter((item: AppRecord) => ['active', 'claimed'].includes(item.status)).length,
    expired: invitations.filter((item: AppRecord) => item.status === 'expired').length,
    restricted: invitations.filter((item: AppRecord) => ['revoked', 'legacy_unverified', 'blocked'].includes(item.status)).length,
  }), [invitations])

  const filteredInvitations = useMemo(() => {
    const query = invitationQuery.trim().toLowerCase()
    const rows = invitations.filter((item: AppRecord) => {
    ...
    return sortedRows(rows, invitationSort, (item: AppRecord, key: string) => {
  ...
  const filteredLogs = useMemo(() => {
    ...
    const rows = logs.filter((item: AppRecord) => {
      ...
      const queryMatch = !query || [
        ...
        item.client_ip,
        item.role,
      ].some((value: any) => String(value || '').replaceAll('_', ' ').toLowerCase().includes(query))
      return roleMatch && resultMatch && queryMatch
    })
    return sortedRows(rows, logSort, (item: AppRecord, key: string) => {
