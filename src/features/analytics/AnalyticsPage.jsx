...
function AnalyticsListToolbar({
  ...
  return <div className="analytics-list-toolbar">
    ...
    <div className="analytics-filter-icons" aria-label="Result filters">
      <FilterIconButton active={outcome === 'all'} label="Show all results" onClick={() => onOutcomeChange('all')}><ListFilterIcon size={16} /></FilterIconButton>
      ...
      <FilterIconButton label={sortDescending ? 'Sort P/L from lowest to highest' : 'Sort P/L from highest to lowest'} onClick={onToggleSort}><SortIcon size={16} descending={sortDescending} /></FilterIconButton>
    </div>
...
function BacktestAnalytics({ dashboard }) {
  ...
  const [assetSortDescending, setAssetSortDescending] = useState(true)
  const [rotationQuery, setRotationQuery] = useState('')
  const [rotationOutcome, setRotationOutcome] = useState('all')
  const [rotationSortDescending, setRotationSortDescending] = useState(true)
  ...
  const assetRows = useMemo(() => {
    ...
    return [...(data?.asset_attribution || [])]
      ...
      .sort((left, right) => {
        const difference = Number(left.total_realized_pnl || 0) - Number(right.total_realized_pnl || 0)
        return assetSortDescending ? -difference : difference
      })
  }, [assetOutcome, assetQuery, assetSortDescending, data])
  ...
  const rotationRows = useMemo(() => {
    ...
    return [...(data?.transition_matrix || [])]
      ...
      .sort((left, right) => {
        const difference = Number(left.total_realized_pnl || 0) - Number(right.total_realized_pnl || 0)
        return rotationSortDescending ? -difference : difference
      })
  }, [data, rotationOutcome, rotationQuery, rotationSortDescending])
  ...
  useEffect(() => { setRotationPage(1) }, [jobId, rotationOutcome, rotationQuery, rotationSortDescending])
  ...
  return <div className="analytics-stack">
    ...
    {!loading && data ? <>
      ...
      <Panel
        kicker="ATTRIBUTION"
        title="Realized result by asset"
        className="analytics-list-panel"
        action={<AnalyticsListToolbar
          query={assetQuery}
          onQueryChange={setAssetQuery}
          outcome={assetOutcome}
          onOutcomeChange={setAssetOutcome}
          sortDescending={assetSortDescending}
          onToggleSort={() => setAssetSortDescending((current) => !current)}
          resultCount={assetRows.length}
          searchPlaceholder="Filter asset"
        />}
      >
        <div className="table-wrap"><table className="dashboard-table analytics-table analytics-table-wide"><thead><tr><th>Asset</th><th>Positions</th><th>Win rate</th><th>Realized P/L</th><th>Avg. return</th><th>Fees</th></tr></thead><tbody>{assetRows.length ? assetRows.map((item) => <tr key={item.asset}><td><strong>{item.asset}</strong></td><td>{item.closed_positions}</td><td>{percent(item.win_rate)}</td><td className={tone(item.total_realized_pnl)}>{money(item.total_realized_pnl)}</td><td className={tone(item.average_position_return)}>{percent(item.average_position_return)}</td><td>{money(item.transaction_fees)}</td></tr>) : <tr><td colSpan="6" className="empty-cell">No assets match the selected filters.</td></tr>}</tbody></table></div>
      </Panel>

      <Panel
        kicker="ROTATION QUALITY"
        title="Origin → destination matrix"
        className="analytics-list-panel"
        action={<AnalyticsListToolbar
          query={rotationQuery}
          onQueryChange={setRotationQuery}
          outcome={rotationOutcome}
          onOutcomeChange={setRotationOutcome}
          sortDescending={rotationSortDescending}
          onToggleSort={() => setRotationSortDescending((current) => !current)}
          resultCount={rotationRows.length}
          searchPlaceholder="Filter origin or destination"
        />}
      >
        <div className="table-wrap"><table className="dashboard-table analytics-table analytics-table-wide"><thead><tr><th>Transition</th><th>Rotations</th><th>Win rate</th><th>Realized P/L</th><th>Avg. return</th><th>Fees</th></tr></thead><tbody>{paginatedRotationRows.length ? paginatedRotationRows.map((item) => <tr key={`${item.from_asset}-${item.to_asset}`}><td><strong>{item.from_asset} → {item.to_asset}</strong></td><td>{item.rotations}</td><td>{percent(item.win_rate)}</td><td className={tone(item.total_realized_pnl)}>{money(item.total_realized_pnl)}</td><td className={tone(item.average_position_return)}>{percent(item.average_position_return)}</td><td>{money(item.transaction_fees)}</td></tr>) : <tr><td colSpan="6" className="empty-cell">No rotations match the selected filters.</td></tr>}</tbody></table></div>
