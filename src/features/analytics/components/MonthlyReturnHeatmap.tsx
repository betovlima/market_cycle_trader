...
function monthNames() {
  ...
  return Array.from({ length: 12 }, (_: any, index: number) => formatter.format(new Date(Date.UTC(2024, index, 1))).replace('.', ''))
}
const TOOLTIP_WIDTH = 248
const TOOLTIP_PADDING = 12

function selectedModeLabel(mode: string, simulationLabel: string = null, referenceLabel: string = null, excessLabel: string = null) {
...
function mapMonthlyReturns(rows: any[], mode: string) {
  const values = new Map()
  let maxAbs = 0

  ;(rows || []).forEach((row: AppRecord) => {
...
function compoundReturn(values: any[]) {
  if (!values.length) return null
  return values.reduce((capital: any, value: any) => capital * (1 + value), 1) - 1
}

function average(values: any[]) {
  return values.length ? values.reduce((total: number, value: any) => total + value, 0) / values.length : null
}

function median(values: any[]) {
  if (!values.length) return null
  const ordered = [...values].sort((left: any, right: any) => left - right)
...
function standardDeviation(values: any[]) {
  ...
  const variance = values.reduce((total: number, value: any) => total + ((value - mean) ** 2), 0) / values.length
  return Math.sqrt(variance)
}

function monthKey(row: AppRecord) {
  const parts = monthParts(row?.month)
  return parts ? `${parts.year}-${String(parts.month).padStart(2, '0')}` : ''
}

function selectedReturn(row: AppRecord, mode: string) {
  const simulation = Number(row?.simulation_return)
  const reference = Number(row?.reference_return)
...
function monthlyContext(rows: any[], target: AppRecord, mode: string) {
  const valid = (rows || []).filter((row: AppRecord) => selectedReturn(row, mode) != null)
  const targetKey = `${target.year}-${String(target.monthNumber).padStart(2, '0')}`
  const index = valid.findIndex((row: AppRecord) => monthKey(row) === targetKey)
  const selectedValues = valid.map((row: AppRecord) => selectedReturn(row, mode))
  const ranked = [...valid].sort((left: any, right: any) => selectedReturn(right, mode) - selectedReturn(left, mode))
  const rank = ranked.findIndex((row: AppRecord) => monthKey(row) === targetKey) + 1
  const currentYearRows = valid.filter((row: AppRecord) => monthParts(row.month)?.year === target.year)
  const ytdRows = currentYearRows.filter((row: AppRecord) => monthParts(row.month)?.month <= target.monthNumber)
  ...
  return {
    ...
    rolling3: compoundReturn(rollingRows.map((row: AppRecord) => selectedReturn(row, mode)).filter(Number.isFinite)),
    ytdSimulation: compoundReturn(ytdRows.map((row: AppRecord) => Number(row.simulation_return)).filter(Number.isFinite)),
    ytdReference: compoundReturn(ytdRows.map((row: AppRecord) => Number(row.reference_return)).filter(Number.isFinite)),
    streak,
    streakTone,
...
function MonthlyReturnDialog({ detail, rows, mode, referenceLabel = null, onClose }: AppRecord) {
  ...
  return createPortal(<div className="monthly-return-dialog-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="monthly-return-dialog" role="dialog" aria-modal="true" aria-label={fullMonthLabel(detail.year, detail.monthNumber)} onMouseDown={(event: any) => event.stopPropagation()}>
...
export function MonthlyReturnHeatmap({ rows, mode, simulationLabel = null, referenceLabel = null, excessLabel = null, onMonthSelect = null }: AppRecord) {
  const [tooltip, setTooltip] = useState(null)
  const [selectedMonth, setSelectedMonth] = useState(null)
  const mapped = useMemo(() => mapMonthlyReturns(rows, mode), [mode, rows])
  const years = useMemo<number[]>(() => [...new Set<number>((rows || [])
    .map((row: AppRecord) => monthParts(row.month)?.year)
    .filter((year: unknown): year is number => typeof year === 'number' && Number.isFinite(year)))]
    .sort((left: number, right: number) => left - right), [rows])

  const hideTooltip = useCallback(() => setTooltip(null), [])
  const showTooltip = useCallback((event: any, data: any) => {
  ...
  return <>
    <div className="analytics-return-heatmap" role="grid" aria-label={tr("Monthly return heatmap")} onMouseLeave={hideTooltip}>
      <div className="analytics-heatmap-head" aria-hidden="true">
        <span />
        {months.map((name: string) => <span key={name}>{name}</span>)}
      </div>

      {years.map((year: number) => <div className="analytics-heatmap-row" key={year}>
        <strong>{year}</strong>
        {months.map((monthName: string, index: number) => {
          ...
          return <button
            ...
            onMouseEnter={(event: any) => showTooltip(event, tooltipData)}
            onFocus={(event: any) => showTooltip(event, tooltipData)}
            onBlur={hideTooltip}
