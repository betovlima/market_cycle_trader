...
export function AnalyticsResponsiveContainer({ children, fallbackHeight = 260 }: AppRecord) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      animationFrame = requestAnimationFrame(() => {
        ...
        setSize((current: any) => current.width === nextWidth && current.height === nextHeight
          ? current
...
export function SectionHeading({ kicker, title, description = '', action = null, hint = null, hintId = '' }: AppRecord) {
...
export function ChartCell({ kicker, title, children, className = '', action = null }: AppRecord) {
...
export function ChartEmpty({ children = 'Not enough observations for this chart.' }: AppRecord) {
  return <div className="analytics-empty">{typeof children === 'string' ? tr(children) : children}</div>
}

export function AnalyticsModeTabs({ value, onChange, items, label }: AppRecord) {
  return <div className="analytics-mode-tabs" role="tablist" aria-label={tr(label)}>
    {items.map((item: AppRecord) => <button
      key={item.value}
      type="button"
      role="tab"
...
export function AnalyticsDragHandle({ label, onDragStart, onDragEnd, onKeyDown }: AppRecord) {
  return <span
...
export function AnalyticsMetric({ label, value, note, tone = '', description = '' }: AppRecord) {
