...
function PerformanceTooltip({ active = false, payload = [], referenceLabel = null }: AppRecord) {
  if (!active || !payload?.length) return null
  const point = payload.find((item: AppRecord) => item?.payload?.timestamp)?.payload
...
export function BacktestPerformanceExplorer({ data, jobId }: AppRecord) {
  ...
  const equityRows = useMemo(() => (data?.equity || [])
    .map((row: AppRecord) => ({ ...row, timestamp_value: analyticsTimestamp(row.timestamp) }))
    .filter((row: AppRecord) => row.timestamp_value !== null)
    .sort((left: any, right: any) => left.timestamp_value - right.timestamp_value), [data?.equity])
  ...
  const performanceRows = useMemo(() => {
    ...
    const firstSimulation = visibleEquityRows
      .map((row: AppRecord) => Number(row.simulation_equity))
      .find(Number.isFinite)
    const firstReference = visibleEquityRows
      .map((row: AppRecord) => Number(row.reference_equity))
      .find(Number.isFinite)

    return visibleEquityRows.map((row: AppRecord) => {
  ...
  const performanceDomain = useMemo(() => {
    ...
    const values = performanceRows
      .flatMap((row: AppRecord) => keys.map((key: string) => Number(row[key])))
  ...
  const visibleMonthlyRows = useMemo(() => {
    ...
    return (data?.monthly_returns || []).filter((row: AppRecord) => {
  ...
  const cardDragHandle = (cardId: string, label: string) => <AnalyticsDragHandle
    label={label}
  ...
  const performanceCards: Record<string, any> = {
        ...
        <AnalyticsResponsiveContainer fallbackHeight={350}>
          <LineChart data={performanceRows}>
            ...
            <XAxis
              ...
              tickFormatter={(value: any) => analyticsAxisLabel(value, visibleSpan)}
              minTickGap={38}
            />
            <YAxis
              domain={performanceDomain}
              tickFormatter={(value: any) => performanceMode === 'value'
  ...
  return <section className="analytics-workspace-section analytics-performance-explorer-section">
    ...
    <div className="analytics-performance-explorer analytics-reorderable-performance">
      {reorderable.layout.map((cardId: string) => <div
        key={cardId}
