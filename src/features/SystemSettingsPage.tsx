...
function historySummary(item: AppRecord) {
...
export function SystemSettingsPage({ onSessionExpired }: AppRecord) {
  ...
  async function saveSettings(event: any) {
    ...
    try {
      ...
      setForm((current: any) => ({ ...current, reason: '' }))
  ...
  async function changeTraderMode(mode: string) {
      ...
      <section className="panel settings-workspace-panel">
          ...
          <form className="settings-form settings-compact-form" onSubmit={saveSettings}>
            <div className="settings-model-execution-grid">
              <ToggleField
                ...
                onChange={(checked: boolean) => setForm({ ...form, enabled: checked })}
              />
              <ToggleField
                ...
                onChange={(checked: boolean) => setForm({ ...form, automatic_training_enabled: checked })}
              />
              <NumberField
                ...
                max="1440"
                step="5"
                onChange={(value: any) => setForm({ ...form, timeout_minutes: value })}
              />
              <div className="settings-reason-field settings-inline-reason">
                ...
                <input id="settings-change-reason" value={form.reason} onChange={(event: any) => setForm({ ...form, reason: event.target.value })} maxLength={500} required />
              </div>
        ...
        <section className="settings-workspace-section trader-control-panel settings-trader-section">
          ...
          {traderHistory.length ? (
            <div className="trader-control-history">
              ...
              {traderHistory.slice(0, 5).map((item: AppRecord, index: number) => (
        ...
        <section className="settings-workspace-section settings-history-section">
          ...
          <div className="settings-history-list">
            {history.length ? history.map((item: AppRecord) => (
...
function ToggleField({ id, label, hint, hintAlign = 'left', checked, disabled = false, onChange }: AppRecord) {
  return (
    <div className={`settings-toggle ${disabled ? 'disabled' : ''}`}>
      ...
      <label className="settings-toggle-switch" htmlFor={id}>
        <input id={id} type="checkbox" checked={checked} disabled={disabled} onChange={(event: any) => onChange(event.target.checked)} aria-label={tr(label)} />
        <i aria-hidden="true" />
      </label>
    </div>
  )
...
function NumberField({ id, label, hint, hintAlign = 'left', value, onChange, ...inputProps }: AppRecord) {
  return (
    <div className="settings-number-field">
      <FieldLabel label={label} hint={hint} hintId={`hint-${id}`} align={hintAlign} />
      <input id={id} type="number" value={value} onChange={(event: any) => onChange(event.target.value)} required {...inputProps} />
    </div>
  )
}

function FieldLabel({ label, hint, hintId, align = 'left' }: AppRecord) {
  return (
...
function RuntimeFact({ label, value, tone = '', hint, hintId }: AppRecord) {
  return (
