...
export function ModelTuningCandidateDetailDialog({ candidate, gateTuning, canViewTuningLogs, logLoading, onOpenLog, onClose }: AppRecord) {
  if (!candidate) return null
  return (
    <div className="model-tuning-candidate-detail-overlay" role="presentation" onMouseDown={(event: any) => { if (event.target === event.currentTarget) onClose() }}>
        ...
        <div className="model-tuning-candidate-detail-body">
          ...
          {candidate.proposal?.promising_region ? (
            <div className="model-tuning-promising-region">
              ...
              <div className="model-tuning-settings-grid">
                {Object.entries(candidate.proposal.promising_region).map(([name, bounds]: any[]) => (
          ...
          {candidate.metrics?.folds?.length ? (
            <div className="model-tuning-fold-grid">
              {candidate.metrics.folds.map((fold: AppRecord) => <div key={fold.fold_id}><span>{tr('Fold')} {fold.fold_id}</span><strong>{pct(fold.strategy_return)}</strong><small>{tr('Max DD')} {pct(fold.maximum_drawdown)}</small></div>)}
            </div>
          ) : null}
        </div>
      </div>
...
export function ModelTuningParametersDialog({ candidate, onClose }: AppRecord) {
  if (!candidate) return null
  return (
    <div className="model-tuning-parameters-overlay" role="presentation" onMouseDown={(event: any) => { if (event.target === event.currentTarget) onClose() }}>
...
export function ModelTuningLogDialog({ view, loading, error, runId, onClose, onCopy, onDownload }: AppRecord) {
  if (!view.open && !loading && !error) return null
  return (
    <div className="model-tuning-log-overlay" role="presentation" onMouseDown={(event: any) => { if (event.target === event.currentTarget) onClose() }}>
