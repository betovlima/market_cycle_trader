...
export function CandidateCardMetric({ candidateId, label, value, tone = '' }) {
...
export function CandidateParametersGrid({ settings }) {
...
export function signedMetricTone(value) {
  const parsed = Number(value)
  if (!Number.isFinite(parsed) || parsed === 0) return 'neutral'
  return parsed > 0 ? 'positive' : 'negative'
}

export function probabilityMetricTone(value) {
  const parsed = Number(value)
...
export function TuningContextLabel({ id, label, description, align = 'left' }) {
  return (
