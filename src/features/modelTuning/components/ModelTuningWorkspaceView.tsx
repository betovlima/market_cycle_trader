...
export function ModelTuningWorkspaceView({ workspace }: AppRecord) {
  const {
    active,
    adaptiveMode,
  ...
  return (
    <section className={`model-tuning-panel model-tuning-workspace ${active ? 'is-running' : ''}`}>
      ...
      <div className="model-tuning-workflow-steps" aria-label={tr('Research workflow')}>
        {['1. BASELINE', '2. RESEARCH', '3. TUNING', '4. RESULTS'].map((label: string, index: number) => <span key={label} className={workflowStepIndex === index ? 'active' : ''}>{tr(label)}</span>)}
      </div>
      ...
      <section className="model-tuning-step model-tuning-step-baseline">
        ...
        <div className="model-tuning-strategy-picker model-tuning-idle-only">
          <label><span>{tr('Search Strategy')}</span><input value={strategySearch} onChange={(event: any) => setStrategySearch(event.target.value)} placeholder={tr('Name, id, kind or status')} disabled={active || busy} /></label>
          <label><span>{tr('Status')}</span><select value={strategyStatusFilter} onChange={(event: any) => setStrategyStatusFilter(event.target.value)} disabled={active || busy}><option value="all">{tr('All statuses')}</option>{strategyStatuses.map((status: string) => <option key={status} value={status}>{tr(status)}</option>)}</select></label>
          <label className="wide"><span>{tr('Strategy catalog')}</span><select value={strategy?.id || ''} onChange={(event: any) => selectTuningStrategy(event.target.value)} disabled={!canStartTuning || active || busy}><option value="">{tr('Select Strategy')}</option>{selectableStrategyCatalog.map((item: AppRecord) => <option key={item.id} value={item.id}>{item.name} · {String(item.status || 'draft').toUpperCase()} · {item.strategy_kind || 'standard'} · r{item.revision}</option>)}</select></label>
        </div>
      ...
      {temporalStrategy && temporalModes.length ? (
        <div className="model-tuning-temporal-mode model-tuning-idle-only">
          ...
          <div className="research-mode-switch">
            {temporalModes.map((item: AppRecord) => (
              <button
                key={item.id}
                type="button"
        ...
        <section className="model-tuning-fold-protocol model-tuning-idle-only">
          ...
          <div className="model-tuning-fold-protocol-grid">
            <label>
              ...
              <input type="number" min={foldMinimum} step="1" value={researchFolds} disabled={foldInputDisabled} onChange={(event: any) => setResearchFolds(event.target.value)} />
            ...
            <label>
              ...
              <input type="number" min={Math.max(foldMinimum, Number(researchFolds) || foldMinimum)} step="1" value={validationFolds} disabled={foldInputDisabled} onChange={(event: any) => setValidationFolds(event.target.value)} />
            ...
            <label>
              ...
              <input type="number" min={Math.max(foldMinimum, Number(validationFolds) || foldMinimum)} step="1" value={certificationFolds} disabled={foldInputDisabled} onChange={(event: any) => setCertificationFolds(event.target.value)} />
      ...
      <div className="model-tuning-method-selector model-tuning-method-selector-compact model-tuning-idle-only">
        <label>
          ...
          >
            {(catalog.methods || []).map((item: AppRecord) => <option key={item.id} value={item.id}>{item.label}</option>)}
          </select>
        </label>
      ...
      <div className="model-tuning-controls">
        <label>
          ...
          <input type="number" min={catalog.candidate_count_min} max={catalog.research_budget_technical_segment_max || catalog.candidate_count_max} step="1" value={candidateCount} disabled={!canStartTuning || !canTune || active || busy} onChange={(event: any) => setCandidateCount(event.target.value)} />
        ...
        <label>
          ...
          <input type="number" min="0" step="1" value={seed} disabled={!canStartTuning || !canTune || active || busy} onChange={(event: any) => setSeed(event.target.value)} />
        </label>
        ...
        <details className="model-tuning-space model-tuning-advanced model-tuning-idle-only">
          ...
          <div className="model-tuning-probability-config">
            <div className="model-tuning-probability-grid">
              <label><span>{tr('Minimum capital improvement (%)')}</span><input type="number" min="0" step="0.1" value={minimumCapitalImprovementPct} disabled={!canStartTuning || active || busy} onChange={(event: any) => setMinimumCapitalImprovementPct(event.target.value)} /></label>
              <label><span>{tr('Sharpe tolerance')}</span><input type="number" min="0" step="0.01" value={sharpeTolerance} disabled={!canStartTuning || active || busy} onChange={(event: any) => setSharpeTolerance(event.target.value)} /></label>
              <label><span>{tr('Drawdown tolerance (pp)')}</span><input type="number" min="0" step="0.1" value={drawdownTolerancePct} disabled={!canStartTuning || active || busy} onChange={(event: any) => setDrawdownTolerancePct(event.target.value)} /></label>
              <label><span>{tr('Minimum worst fold (%)')}</span><input type="number" step="0.1" value={minimumWorstFoldPct} disabled={!canStartTuning || active || busy} onChange={(event: any) => setMinimumWorstFoldPct(event.target.value)} /></label>
              <label className="model-tuning-checkbox-control"><span>{tr('Adaptive early stopping')}</span><input type="checkbox" checked={adaptiveStoppingEnabled} disabled={!canStartTuning || active || busy} onChange={(event: any) => setAdaptiveStoppingEnabled(event.target.checked)} /></label>
              <label><span>{tr('No-improvement trials')}</span><input type="number" min="10" step="10" value={noImprovementTrialLimit} disabled={!canStartTuning || active || busy || !adaptiveStoppingEnabled} onChange={(event: any) => setNoImprovementTrialLimit(event.target.value)} /></label>
              <label><span>{tr('Meaningful improvement (%)')}</span><input type="number" min="0" step="0.05" value={minimumMeaningfulImprovementPct} disabled={!canStartTuning || active || busy || !adaptiveStoppingEnabled} onChange={(event: any) => setMinimumMeaningfulImprovementPct(event.target.value)} /></label>
            </div>
          </div>
        </details>
      ) : null}
      ...
      <details className="model-tuning-space model-tuning-idle-only">
        ...
        <div className="model-tuning-space-grid">
          {(effectivePlan?.search_space || catalog.search_space || []).map((field: AppRecord) => (
            <div key={field.name}>
        ...
        <div className="model-tuning-run">
          <div className="model-tuning-progress-row">
            <div>
              ...
              <span>{tr(run.status)} · {run.research_completed_candidates ?? run.completed_candidates}/{run.research_total_candidates ?? run.total_candidates} {tr('completed')}{run.cancelled_candidates ? ` · ${run.cancelled_candidates} ${tr('cancelled')}` : ''} · {tr(run.tuning_scope_label || catalog?.tuning_scope_label || '')} · {(catalog.methods || []).find((item: AppRecord) => item.id === run.method)?.label || run.method}</span>
          ...
          {run.active_candidate_ids?.length ? <small>{tr(run.status === 'stop_requested' ? 'Cancelling candidate' : 'Active candidates')} · {run.active_candidate_ids.map((id: string) => `#${id}`).join(', ')}</small> : null}
        </div>
      ) : null}
      ...
      {visibleCandidates.length ? (
        <div className="model-tuning-results-wrap">
          ...
          <div className="model-tuning-candidate-grid" aria-label={tr('Candidate ranking')}>
            {visibleCandidates.map((candidate: AppRecord) => (
