    ...
    <section className="regime-clustering-dialog" role="dialog" aria-modal="true" aria-label={fullMonthLabel(row.month)} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="regime-clustering-dialog-columns">
        ...
        <article>
          ...
          <div className="regime-clustering-similar-list">{(row.similar_months || []).map((similar) => <div key={similar.month} className="regime-clustering-similar-item">
...
function ScatterPlot({ monthly, clusters, selection, onSelectMonth, onSelectCentroid }) {
  ...
  const centroids = useMemo(() => {
    ...
    return [...byCluster.values()].map((bucket) => ({
      ...
      cluster: (clusters || []).find((item) => Number(item.cluster_id) === bucket.cluster_id) || null,
    }))
  }, [points, clusters])
  ...
  return <div className="regime-clustering-card regime-clustering-scatter-card">
    ...
    <div className="regime-clustering-centroid-legend"><span className="regime-clustering-centroid-symbol" aria-hidden="true">◆</span><small>{tr('Diamond markers identify the centroid of each cluster. Click a centroid to inspect its aggregate profile.')}</small></div>
    <svg viewBox={`0 0 ${width} ${height}`} className="regime-clustering-scatter" role="img" aria-label={tr('Monthly similarity map')}>
      ...
      {points.map((point) => {
        ...
        const selected = selection?.type === 'month' && selection?.data?.month === point.month
        ...
        return <g
          ...
          onClick={(event) => { event.stopPropagation(); onSelectMonth?.(point) }}
          role="button"
          tabIndex={0}
          aria-label={`${fullMonthLabel(point.month)} · ${tr('Cluster')} ${Number(point.cluster_id) + 1}`}
          onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelectMonth?.(point) }}
        >
      ...
      {centroids.map((centroid) => {
        ...
        const selected = selection?.type === 'centroid' && Number(selection?.data?.cluster_id) === Number(centroid.cluster_id)
        return <g
          key={`centroid-${centroid.cluster_id}`}
          className={`regime-clustering-centroid ${colorClass} ${selected ? 'selected' : ''}`}
          onClick={(event) => { event.stopPropagation(); onSelectCentroid?.(centroid.cluster) }}
          role="button"
          tabIndex={0}
          aria-label={`${tr('Centroid')} · ${tr('Cluster')} ${Number(centroid.cluster_id) + 1}`}
          onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelectCentroid?.(centroid.cluster) }}
        >
          <polygon points={`${x},${y - (selected ? 12 : 10)} ${x + (selected ? 12 : 10)},${y} ${x},${y + (selected ? 12 : 10)} ${x - (selected ? 12 : 10)},${y}`} />
          <text x={x + 14} y={y - 12}>{`${tr('Centroid')} #${Number(centroid.cluster_id) + 1}`}</text>
        </g>
      })}
    </svg>
  </div>
  ...
  return createPortal(<div className="regime-clustering-dialog-backdrop" onMouseDown={onClose} role="presentation">
    <section className="regime-clustering-dialog regime-clustering-hint-dialog" role="dialog" aria-modal="true" aria-label={tr('What does this clustering group?')} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="regime-clustering-hint-content">
        ...
        <p>{tr('Centroids are interactive. Click a diamond to inspect the aggregate profile of that cluster in the detail panel.')}</p>
      </div>
    </section>
  </div>, document.body)
}

function SelectionPanel({ selection, onOpenMonth, onOpenCluster }) {
  if (!selection) return <div className="regime-clustering-card regime-clustering-selection-card regime-clustering-selection-empty"><span className="panel-kicker">{tr('SELECTION DETAILS')}</span><strong>{tr('Nothing selected')}</strong><span className="regime-clustering-muted">{tr('Click a month or centroid in the similarity map, or a month in the timeline, to inspect it.')}</span></div>

  if (selection.type === 'centroid') {
    const cluster = selection.data || {}
    const topFeatures = Object.entries(cluster.feature_zscores || {})
      .filter(([, value]) => Number.isFinite(Number(value)))
      .sort((left, right) => Math.abs(Number(right[1])) - Math.abs(Number(left[1])))
      .slice(0, 5)
    return <div className="regime-clustering-card regime-clustering-selection-card regime-clustering-centroid-detail">
      <div className="regime-clustering-selection-head"><div><span className="panel-kicker">{tr('SELECTED CENTROID')}</span><h4>{tr('Cluster')} #{Number(cluster.cluster_id) + 1}</h4></div><span className={`regime-clustering-centroid-badge ${COLORS[Number(cluster.cluster_id) % COLORS.length]}`}>◆</span></div>
      <div className="regime-clustering-selection-metrics">
        <Metric label="Months" value={String(cluster.months ?? '—')} />
        <Metric label="Average return" value={percent(cluster.average_return, 1)} tone={Number(cluster.average_return) >= 0 ? 'positive' : 'negative'} />
        <Metric label="Negative rate" value={percent(cluster.negative_rate, 1)} />
        <Metric label="Severe months" value={String(cluster.severe_negative_months ?? '—')} />
        <Metric label="Dominant outcome" value={outcomeLabel(cluster.dominant_outcome)} />
      </div>
      <strong className="regime-clustering-subtitle">{tr('Top cluster features')}</strong>
      <div className="regime-clustering-centroid-features">{topFeatures.map(([key, value]) => <div key={key}><span>{featureLabel(key)}</span><strong>{number(value, 2)} z</strong></div>)}</div>
      <button type="button" className="regime-clustering-detail-button" onClick={() => onOpenCluster?.(cluster)}>{tr('View cluster details')}</button>
    </div>
  }

  const row = selection.data
  ...
  return <div className="regime-clustering-card regime-clustering-selection-card">
    ...
    <button type="button" className="regime-clustering-detail-button" onClick={() => onOpenMonth?.(row)}>{tr('View full period details')}</button>
  </div>
}
...
function Timeline({ monthly, selection, onSelect }) {
  ...
  return <div className="regime-clustering-card regime-clustering-timeline-card">
    ...
    <div className="regime-clustering-timeline-grid">
      ...
      {years.map((year) => <div className="regime-clustering-timeline-row" key={year}><strong>{year}</strong>{Array.from({ length: 12 }, (_, index) => {
        ...
        const selected = selection?.type === 'month' && selection?.data?.month === row.month
        return <button type="button" key={key} className={`regime-clustering-timeline-cell ${COLORS[Number(row.cluster_id) % COLORS.length]} ${Number(row.official_return) < 0 ? 'negative' : ''} ${selected ? 'selected' : ''}`} onClick={() => onSelect?.(row)} aria-label={`${fullMonthLabel(row.month)} · ${tr('Cluster')} ${Number(row.cluster_id) + 1} · ${percent(row.official_return, 1)}`}>
  ...
  return <section className="regime-clustering-panel">
    ...
    <div className="regime-clustering-main-grid">
      <ScatterPlot
        ...
        clusters={clusters}
        selection={selection}
        onSelectMonth={(row) => setSelection({ type: 'month', data: row })}
        onSelectCentroid={(cluster) => cluster && setSelection({ type: 'centroid', data: cluster })}
      />
      <SelectionPanel selection={selection} onOpenMonth={setDetailMonth} onOpenCluster={setDetailCluster} />
    </div>

    <ClusterProfileHeatmap clusters={clusters} features={topFeatures} />
    <Timeline monthly={monthly} selection={selection} onSelect={(row) => setSelection({ type: 'month', data: row })} />
    ...
    <div className="regime-clustering-card">
      ...
      <div className="regime-clustering-clusters">{clusters.map((cluster) => <button type="button" key={cluster.cluster_id} className="regime-clustering-cluster-card" onClick={() => setDetailCluster(cluster)}>
    ...
    <ClusterDialog cluster={detailCluster} onClose={() => setDetailCluster(null)} />
