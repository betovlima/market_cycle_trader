...
function familyLabel(value: any) {
...
function Metric({ label, value, tone = '' }: AppRecord) {
  return <div className={`winner-risk-metric ${tone}`}><span>{label}</span><strong>{value}</strong></div>
}

export function WinnerTransitionRiskSearchPanel({ study, runId, processingId, canRun = false, showRunButton = true, refreshToken = 0, onChange }: AppRecord) {
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/temporal-intelligence/${encodeURIComponent(runId)}/winner-transition-risk-search/latest?${query.toString()}`)
      .then((payload: AppRecord) => { if (active && payload?.id) setSearch(payload) })
  ...
  const model = useMemo(() => {
    ...
    const baselineMonthMap = new Map<any, AppRecord>(baselineMonths.map((row: AppRecord) => [row.month, row]))
    const shadowMonthMap = new Map<any, AppRecord>(shadowMonths.map((row: AppRecord) => [row.month, row]))
    const worstBaseline = [...baselineMonths].sort((left: any, right: any) => Number(left.return ?? 0) - Number(right.return ?? 0)).slice(0, 6)
    const worstShadow = [...shadowMonths].sort((left: any, right: any) => Number(left.return ?? 0) - Number(right.return ?? 0)).slice(0, 6)
    const monthKeys = [...new Set([...worstBaseline, ...worstShadow].map((row: AppRecord) => row.month))]
    const monthRows = monthKeys
      ...
      .map((row: AppRecord) => ({ ...row, delta: row.shadowReturn == null || row.baselineReturn == null ? null : Number(row.shadowReturn) - Number(row.baselineReturn) }))
      .sort((left: any, right: any) => Math.min(Number(left.baselineReturn ?? 0), Number(left.shadowReturn ?? 0)) - Math.min(Number(right.baselineReturn ?? 0), Number(right.shadowReturn ?? 0)))
      .slice(0, 10)
    ...
    {search ? <>
      ...
      {model.monthRows.length ? <div className="temporal-table-shell winner-risk-table-shell">
        <table className="temporal-table winner-risk-table winner-risk-month-table">
          ...
          <tbody>{model.monthRows.map((row: AppRecord) => <tr key={row.month}>
      ...
      {model.familyRows.length ? <div className="temporal-table-shell winner-risk-table-shell">
        <table className="temporal-table winner-risk-table">
          ...
          <tbody>{model.familyRows.map((row: AppRecord) => <tr key={row.family}>
      ...
      {model.outerRows.length ? <div className="temporal-table-shell winner-risk-table-shell">
        <table className="temporal-table winner-risk-table winner-risk-outer-table">
          ...
          <tbody>{model.outerRows.map((row: AppRecord) => <tr key={row.test_year}>
      ...
      {model.riskRows.length ? <div className="temporal-table-shell winner-risk-table-shell">
        <table className="temporal-table winner-risk-table winner-risk-events-table">
          ...
          <tbody>{model.riskRows.slice(0, 15).map((row: AppRecord, index: number) => <tr key={`${row.transition_key}-${index}`}>
