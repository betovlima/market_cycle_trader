...
function metric(candidate: AppRecord, baseline: any, key: string, format: any, digits: any = 2) {
...
function stageDone(search: AppRecord, stage: any) {
...
export function TemporalPolicySearchPanel({ study, runId, processingId = null, canRun = false, showActions = true, onChange }: AppRecord) {
  ...
  return <section className="temporal-policy-search">
    ...
    <div className="temporal-policy-budget">
      <label><span>{tr('Hypercube trials')}</span><input type="number" min="4" step="1" value={lhsTrials} onChange={(event: any) => setLhsTrials(event.target.value)} disabled={Boolean(busy)} /></label>
      <label><span>{tr('CARO trials / outer fold')}</span><input type="number" min="1" step="1" value={caroTrials} onChange={(event: any) => setCaroTrials(event.target.value)} disabled={Boolean(busy)} /></label>
      <label><span>{tr('Seed')}</span><input type="number" min="0" step="1" value={seed} onChange={(event: any) => setSeed(event.target.value)} disabled={Boolean(busy)} /></label>
    ...
    {sampled ? <div className="temporal-policy-card">
      ...
      <div className="temporal-policy-table-wrap"><table className="temporal-policy-table"><thead><tr><th>{tr('Outer fold')}</th><th>{tr('Research folds')}</th><th>{tr('Candidates')}</th><th>{tr('Champion capital')}</th><th>{tr('Champion Sharpe')}</th><th>{tr('Search utility')}</th></tr></thead><tbody>{samplingOuter.map((item: AppRecord) => <tr key={item.outer_fold_id}><td>{item.outer_fold_id}</td><td>{(item.inner_fold_ids || []).join(', ')}</td><td>{number(item.evaluated_count, 0)}</td><td>{money(item.champion?.metrics?.ending_capital)}</td><td>{number(item.champion?.metrics?.sharpe, 3)}</td><td>{number(item.champion?.metrics?.search_utility, 4)}</td></tr>)}</tbody></table></div>
    </div> : null}
    ...
    {caroDone ? <div className="temporal-policy-card">
      ...
      <div className="temporal-policy-table-wrap"><table className="temporal-policy-table"><thead><tr><th>{tr('Outer fold')}</th><th>{tr('Adaptive trials')}</th><th>{tr('Champion capital')}</th><th>{tr('Champion Sharpe')}</th><th>{tr('Search utility')}</th><th>{tr('CASH guard')}</th></tr></thead><tbody>{caroOuter.map((item: AppRecord) => <tr key={item.outer_fold_id}><td>{item.outer_fold_id}</td><td>{number(item.completed_count, 0)}</td><td>{money(item.champion?.metrics?.ending_capital)}</td><td>{number(item.champion?.metrics?.sharpe, 3)}</td><td>{number(item.champion?.metrics?.search_utility, 4)}</td><td>{Number(item.champion?.settings?.late_exit_cash_guard) === 1 ? tr('Enabled') : tr('Off')}</td></tr>)}</tbody></table></div>
    </div> : null}
    ...
    {validated ? <div className="temporal-policy-card">
      ...
      <div className="temporal-policy-table-wrap"><table className="temporal-policy-table"><thead><tr><th>{tr('Outer fold')}</th><th>{tr('Baseline return')}</th><th>{tr('Candidate return')}</th><th>{tr('Return delta')}</th><th>{tr('Sharpe delta')}</th><th>{tr('MaxDD delta')}</th><th>{tr('CASH guards')}</th></tr></thead><tbody>{outerResults.map((item: AppRecord) => <tr key={item.outer_fold_id}><td>{item.outer_fold_id}</td><td>{percent(item.baseline_outer_metrics?.strategy_return, 2)}</td><td>{percent(item.candidate_outer_metrics?.strategy_return, 2)}</td><td>{percent(item.return_delta, 2)}</td><td>{number(item.sharpe_delta, 3)}</td><td>{percent(item.maximum_drawdown_delta, 2)}</td><td>{number(item.late_exit_cash_guard_count, 0)}</td></tr>)}</tbody></table></div>
    ...
    {log.length ? <div className="temporal-policy-card">
      ...
      <div className="temporal-policy-log">{log.map((item: AppRecord, index: number) => <div key={`${item.at}-${index}`}><span>{shortDateTime(item.at)}</span><strong>{tr(item.stage)}</strong><em>{tr(item.outcome)}</em><p>{tr(item.message)}</p></div>)}</div>
    </div> : null}
  </section>
}
