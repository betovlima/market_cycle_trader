...
function ShadowTooltip({ active = false, payload = [], mode, hasCalibrated }: AppRecord) {
...
function pathMap(rows: any[]) {
  return new Map((rows || []).map((row: AppRecord) => [String(row.timestamp || ''), Number(row.value)]))
}

export function TransitionShadowCapitalChart({ result, confidenceResult = null, chartKey = '' }: AppRecord) {
  ...
  const equityRows = useMemo(() => {
    ...
    return (baselineRows || [])
      ...
      .filter((row: AppRecord) => row.timestamp_value !== null && Number.isFinite(row.baseline))
      .sort((left: any, right: any) => left.timestamp_value - right.timestamp_value)
  ...
  const domain = useMemo(() => {
    ...
    const values = visibleEquityRows.flatMap((row: AppRecord) => keys.map((key: string) => Number(row[key]))).filter(Number.isFinite)
    ...
    >
      <AnalyticsResponsiveContainer fallbackHeight={350}>
        <LineChart data={visibleEquityRows}>
          ...
          <XAxis dataKey="timestamp_value" type="number" domain={['dataMin', 'dataMax']} tickFormatter={(value: any) => analyticsAxisLabel(value, visibleSpan)} minTickGap={38} />
          <YAxis domain={domain} tickFormatter={(value: any) => `$${Math.round(value / 1000)}k`} />
