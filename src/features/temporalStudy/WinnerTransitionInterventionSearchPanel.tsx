...
function shadowEquity(replay: any) {
  return (((replay?.equity || {}).shadow) || []).map((row: AppRecord) => ({
    timestamp: row.timestamp,
    simulation_equity: row.value,
  }))
}

function shadowSeriesOption(key: string, label: string, replay: any) {
...
export function WinnerTransitionInterventionSearchPanel({ study, runId, processingId, riskSearch, confidenceSearch = null, canRun = false, showRunButton = true, refreshToken = 0, onChange }: AppRecord) {
  ...
  return <section className="winner-transition-risk-search winner-transition-intervention-search">
    ...
    {search ? <>
      ...
      {(() => {
        ...
        const preferred = series.some((item: AppRecord) => item.key === 'confidence_calibrated')
          ? 'confidence_calibrated'
          : (series.some((item: AppRecord) => item.key === 'walk_forward') ? 'walk_forward' : 'control')
