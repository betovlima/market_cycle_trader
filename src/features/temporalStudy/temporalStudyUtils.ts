...
export function periodFromSnapshot(value: any) {
...
export function periodIsValid(start: any, end: any) {
  if (!/^\d{4}-\d{2}$/.test(start || '') || !/^\d{4}-\d{2}$/.test(end || '')) return false
  return end >= start
}

export function filterAnalyticsByPeriod(payload: AppRecord, start: any, end: any) {
  ...
  return {
    ...payload,
    equity: (payload?.equity || []).filter((row: AppRecord) => inPeriod(row.timestamp || row.recorded_at)),
    rotations: (payload?.rotations || []).filter((row: AppRecord) => inPeriod(row.executed_at)),
    monthly_returns: (payload?.monthly_returns || []).filter((row: AppRecord) => {
...
export function monthsWithData(analytics: AppRecord) {
...
export function temporalStudyParameters(run: AppRecord, processing: AppRecord, start: any, end: any) {
...
function crc32(bytes: Uint8Array) {
  if (!crcTable) {
    crcTable = Array.from({ length: 256 }, (_: any, value: any) => {
      let crc = value
...
export async function saveJsonZip(filename: string, payload: AppRecord) {
...
export function attachDecisionContexts(rotations: AppRecord[], contexts: AppRecord[]) {
  const exact = new Map<string, AppRecord>()
  ...
  return (rotations || []).map((rotation: any) => {
    ...
    const matched = exact.get(`${day}|${fromAsset}|${toAsset}`)
      || (byDay.get(day) || []).find((context: AppRecord) => normalizedAsset(context?.target_symbol) === toAsset)
      || (byDay.get(day) || [])[0]
      || null
...
export function attachMonthlyDecisionContexts(details: AppRecord, contexts: AppRecord[]) {
  const result: AppRecord = {}
  for (const [month, detail] of Object.entries(details || {}) as [string, AppRecord][]) {
    result[month] = {
      ...detail,
...
export function attachWinnerTransitionAttributions(rotations: AppRecord[], attributions: AppRecord[]) {
  const exact = new Map<string, AppRecord>()
  const byDay = new Map<string, AppRecord[]>()
  ...
  return (rotations || []).map((rotation: any) => {
    ...
    const toAsset = normalizedAsset(rotation?.to_asset)
    const matched = exact.get(`${day}|${fromAsset}|${toAsset}`)
      || (byDay.get(day) || []).find((item: AppRecord) => normalizedAsset(item?.to_asset) === toAsset)
      || null
...
export function attachMonthlyWinnerTransitionAttributions(details: AppRecord, attributions: AppRecord[]) {
  const result: AppRecord = {}
  for (const [month, detail] of Object.entries(details || {}) as [string, AppRecord][]) {
    result[month] = {
      ...detail,
