...
function candidateSeries(key: string, label: string, candidate: AppRecord) {
...
export function WinnerTransitionStatefulReplayPanel({ study, runId, processingId, confidenceSearch, canRun = false, showRunButton = true, refreshToken = 0, onChange }: AppRecord) {
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/temporal-intelligence/${encodeURIComponent(runId)}/winner-transition-stateful-replay/latest?${query.toString()}`)
      .then((payload: AppRecord) => { if (active && payload?.id) setResult(payload) })
    ...
    {result ? <>
      ...
      {model.parity?.status === 'passed' ? (() => {
        ...
        return series.length > 1 ? <MonthlyCapitalMovementHeatmap
          ...
          defaultSeriesKey={series.some((item: AppRecord) => item.key === 'stateful_b') ? 'stateful_b' : 'stateful_a'}
        /> : null
      })() : null}
    </> : null}
  </section>
