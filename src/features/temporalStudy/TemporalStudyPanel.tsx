...
function Parameter({ label, value }: AppRecord) {
...
function storedPeriodKey(runId: string, processingId: string) {
  if (!runId || !processingId) return ''
  return `mct.temporal-study.period.${processingId}.${runId}`
}

function readStoredPeriod(runId: string, processingId: string, fallback: any) {
...
function writeStoredPeriod(runId: string, processingId: string, value: any) {
  const key = storedPeriodKey(runId, processingId)
...
export function TemporalStudyPanel({ run = null, processing = null, canRun = false, canExport = false }: AppRecord) {
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/temporal-intelligence/history?limit=30`)
      .then((payload: AppRecord) => {
        const completed = (payload?.items || []).find((item: AppRecord) => String(item?.status || '').toLowerCase() === 'completed')
        if (!active || !completed?.id) return null
        return apiFetch(`${API}/temporal-intelligence/${encodeURIComponent(completed.id)}`)
      })
      .then((value: any) => { if (active && value?.result) setSourceRun(value) })
      .catch(() => { if (active) setSourceRun(null) })
    return () => { active = false }
  }, [run?.id, run?.result, run?.status])

  async function fetchStudyData(periodStart: any, periodEnd: any, executedAt: any, { includeTransition = false }: AppRecord = {}) {
  ...
  useEffect(() => {
    ...
    fetchStudyData(stored.start, stored.end, stored.executed_at)
      .then((value: any) => { if (active) setStudy(value) })
      .catch((requestError: ErrorLike) => { if (active) setError(tr(requestError?.message || 'Unable to load the latest study result.')) })
  ...
  const horizons = (effectiveRun?.horizons || effectiveRun?.result?.horizons || []).map((item: AppRecord) => `${item}d`).join(' · ')
  ...
  async function executeFullAnalysis() {
    ...
    try {
      ...
      setPipelineRefresh((current: any) => current + 1)
    } catch (requestError) {
  ...
  return <section className="temporal-study-panel">
    ...
    <div className="temporal-study-controls">
      <label>
        ...
        <input type="month" value={startMonth} onChange={(event: any) => setStartMonth(event.target.value)} />
      </label>
      <label>
        <span>{tr('Period to')}</span>
        <input type="month" value={endMonth} onChange={(event: any) => setEndMonth(event.target.value)} />
      </label>
