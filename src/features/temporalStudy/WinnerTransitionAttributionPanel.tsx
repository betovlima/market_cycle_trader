...
function rotationValueAdded(row: AppRecord) {
  return finite(row?.rotation_value_added)
}

function windowValue(row: AppRecord, key: number, field: string) {
  return row?.winner_transition?.trajectory?.windows?.[String(key)]?.[field] ?? null
}

function temporalWindowValue(row: AppRecord, key: number, field: string, measure: string = 'latest') {
...
export function WinnerTransitionAttributionPanel({ rotations = [] }: AppRecord) {
  const model = useMemo(() => {
    const attributed = (rotations || []).filter((row: AppRecord) => row?.winner_transition)
    const diagnostic = attributed.filter((row: AppRecord) => rotationValueAdded(row) != null)
    const helpful = diagnostic.filter((row: AppRecord) => rotationValueAdded(row) > 0)
    const harmful = diagnostic.filter((row: AppRecord) => rotationValueAdded(row) < 0)
    const flat = diagnostic.filter((row: AppRecord) => rotationValueAdded(row) === 0)
    const sortedHarmful = [...harmful].sort((left: any, right: any) => rotationValueAdded(left) - rotationValueAdded(right))
    const meanHarmful = harmful.length
      ? harmful.reduce((total: number, row: AppRecord) => total + rotationValueAdded(row), 0) / harmful.length
      : null
    return {
      attributed,
      diagnostic,
  ...
  return <section className="winner-transition-attribution">
    ...
    {model.rows.length ? <div className="temporal-table-shell winner-transition-table-shell">
      <table className="temporal-table winner-transition-table">
        ...
        <tbody>{model.rows.map((row: AppRecord, index: number) => {
