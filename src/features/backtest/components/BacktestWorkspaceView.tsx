...
import { BacktestIcon, ListFilterIcon, PlayIcon } from '../../../shared/components/Icons'
...
import { toggleSort } from '../backtestUtils'
...
export function BacktestWorkspaceView({ workspaceView }: AppRecord) {
  const {
          ...
          <div className="backtest-research-lab-stack">
            ...
            {canViewTuning && researchLabMode === 'tuning' ? (
              <ModelTuningPanel
                ...
                onStrategyModelSaved={(updated: any) => setSelectedStrategyModel(updated?.research_model_configuration || updated?.research_model || null)}
        ...
        <section className="backtest-workspace-section backtest-history-section">
          ...
          <div className="table-wrap backtest-table-wrap">
            <table className="dashboard-table backtest-sortable-table">
              <thead>
                <tr>
                  <SortableHeader label={tr("Date")} field="created_at" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.created_at} />
                  <SortableHeader label={tr("Test")} field="strategy_profile_name" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.strategy_profile_name} />
                  {canViewResearchModels ? <SortableHeader label={tr("Model")} field="research_model_label" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} /> : null}
                  <SortableHeader label={tr("Status")} field="status" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.status} />
                  <SortableHeader label={tr("Total Return")} field="simulation_return" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.simulation_return} />
                  <SortableHeader label={tr("Sharpe Ratio")} field="sharpe" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.sharpe} />
                  <SortableHeader label={tr("Max Drawdown")} field="maximum_drawdown" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.maximum_drawdown} />
                  <SortableHeader label={tr("Rotations")} field="position_changes" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.position_changes} />
                  <SortableHeader label={tr("Duration")} field="duration_seconds" sort={historySort} onSort={(key: string) => setHistorySort((current: any) => toggleSort(current, key))} hint={HISTORY_HINTS.duration_seconds} />
                </tr>
              </thead>
              <tbody>
                {paginatedHistoryRows.length ? paginatedHistoryRows.map((item: AppRecord) => (
