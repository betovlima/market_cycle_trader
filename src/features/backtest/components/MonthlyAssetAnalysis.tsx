...
function normalizedAsset(value: any) {
  const asset = String(value || 'CASH').trim().toUpperCase()
  return asset || 'CASH'
}

function extent(values: any[]) {
  const finite = values.map(finiteNumber).filter((value: any) => value !== null)
...
function chartCoordinates(points: any[], valueKey: string, domain: any): AppRecord[] {
  ...
  return points.flatMap((point: AppRecord) => {
...
function svgPath(points: any[]) {
  return points.map((point: AppRecord, index: number) => `${index ? 'L' : 'M'} ${point.x.toFixed(2)} ${point.y.toFixed(2)}`).join(' ')
}

function nearestPrice(prices: any, timestamp: any) {
...
function indexedSeries(rows: any[], valueKey: string) {
  const clean = (rows || []).flatMap((row: AppRecord) => {
  ...
  const base = clean.find((row: AppRecord) => row.value !== 0)?.value
  if (!base) return []
  return clean.map((row: AppRecord) => ({ timestamp: row.timestamp, value: row.value / base * 100 }))
}
...
function eventRows(movements: any, asset: any, prices: any) {
  const rows = []
...
function PriceAndTradeChart({ detail, asset }: AppRecord) {
  ...
  const model = useMemo(() => {
    const timestamps = prices.map((row: AppRecord) => timestampValue(row.timestamp)).filter((value: any) => value !== null)
    if (!timestamps.length) return null
    const values = [...prices.map((row: AppRecord) => row.close), ...events.map((row: AppRecord) => row.price)]
    ...
    const eventPoints = chartCoordinates(events.map((event: any) => ({ ...event, value: event.price })), 'value', domain)
  ...
  const innerWidth = CHART_WIDTH - CHART_PADDING.left - CHART_PADDING.right
  const heldSegments = (detail?.position_segments || []).filter((segment: any) => normalizedAsset(segment.asset) === asset.symbol)
  ...
  return <div className="rotation-asset-price-chart-wrap">
    <svg className="rotation-asset-price-chart" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} role="img" aria-label={`${asset.symbol} · ${tr('Price and trade events')}`}>
      ...
      {heldSegments.map((segment: any, index: number) => {
      ...
      {model.eventPoints.map((point: AppRecord, index: number) => {
...
function StrategyAssetComparisonChart({ detail, asset }: AppRecord) {
  ...
  const model = useMemo(() => {
    ...
    const timestamps = allRows.map((row: AppRecord) => timestampValue(row.timestamp)).filter((value: any) => value !== null)
    const valueExtent = extent(allRows.map((row: AppRecord) => row.value))
...
function AllocationTimeline({ detail }: AppRecord) {
  ...
  return <div className="rotation-allocation-timeline">
    ...
    <div className="rotation-allocation-track">
      {detail.position_segments.map((segment: any, index: number) => {
...
export function MonthlyAssetAnalysis({ jobId, processingId = null, month, detailUrl = null }: AppRecord) {
  ...
  const asset = useMemo(() => (detail?.assets || []).find((item: AppRecord) => item.symbol === selectedAsset) || detail?.assets?.[0] || null, [detail?.assets, selectedAsset])
  ...
  return <section className="rotation-month-asset-analysis">
    <div className="rotation-month-asset-analysis-heading">
      ...
      {detail?.assets?.length ? <div className="rotation-asset-tabs" role="tablist" aria-label={tr('Operated assets')}>
        {detail.assets.map((item: AppRecord) => <button key={item.symbol} type="button" className={item.symbol === asset?.symbol ? 'active' : ''} onClick={() => setSelectedAsset(item.symbol)}>{item.symbol}</button>)}
      </div> : null}
    </div>
