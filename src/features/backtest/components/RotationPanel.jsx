      ...
      <div className="rotation-month-dialog-table-wrap">
        ...
        <div className="table-wrap">
          <table className="dashboard-table rotation-month-dialog-table">
            <thead><tr><th>{tr('Executed')}</th><th>{tr('Sold')}</th><th>{tr('Bought')}</th><th>{tr('Holding')}</th><th>{tr('Position Return')}</th><th>{tr('Realized P/L')}</th><th>{tr('Fees')}</th></tr></thead>
            <tbody>{month.movements.length ? month.movements.map((item, index) => <tr key={`${item.executed_at}-${index}`}>
            ...
            </tr>) : <tr><td colSpan="7" className="empty-cell">{tr('No capital movements in this month.')}</td></tr>}</tbody>
          </table>
        </div>
      </div>
    </section>
...
function MonthlyCapitalMovementHeatmap({ rotations, equity }) {
  ...
  const model = useMemo(() => monthMovementModel(rotations, equity), [rotations, equity])
  ...
  return <>
    <article className="rotation-monthly-heatmap-panel">
      ...
      <div className="rotation-monthly-heatmap" role="grid" aria-label={tr('Monthly Capital Movement Heatmap')} onMouseLeave={hideTooltip}>
        <div className="rotation-monthly-heatmap-head" aria-hidden="true"><span />{months.map((name) => <span key={name}>{name}</span>)}</div>
        {model.years.map((year) => <div className="rotation-monthly-heatmap-row" key={year}>
          <strong>{year}</strong>
