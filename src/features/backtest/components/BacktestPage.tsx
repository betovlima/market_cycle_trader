...
export function BacktestPage({ workspace, capabilities = {}, onSessionExpired }: AppRecord) {
  ...
  const [researchExecutionModels, setResearchExecutionModels] = useState<AppRecord>({})
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/admin/model-research/executions?limit=50`)
      .then((value: any) => {
        ...
        setResearchExecutionModels(Object.fromEntries(items.filter((item: AppRecord) => item?.id).map((item: AppRecord) => [item.id, item])))
      })
      .catch(() => {
  ...
  const historyRows = useMemo(() => {
    ...
    const rows = (dashboard?.recent_backtests || []).map((item: AppRecord) => ({
      ...item,
    ...
    return sortRows(rows, historySort, {
      created_at: (item: AppRecord) => Date.parse(item.created_at || '') || 0,
      strategy_profile_name: (item: AppRecord) => String(item.strategy_profile_name || 'Unknown test'),
      research_model_label: (item: AppRecord) => String(item.research_model_label || 'Baseline'),
      status: (item: AppRecord) => String(item.status || ''),
      simulation_return: (item: AppRecord) => item.metrics?.simulation_return == null ? null : Number(item.metrics.simulation_return),
      sharpe: (item: AppRecord) => item.metrics?.sharpe == null ? null : Number(item.metrics.sharpe),
      maximum_drawdown: (item: AppRecord) => item.metrics?.maximum_drawdown == null ? null : Number(item.metrics.maximum_drawdown),
      position_changes: (item: AppRecord) => item.metrics?.position_changes == null ? null : Number(item.metrics.position_changes),
      duration_seconds: (item: AppRecord) => item.duration_seconds == null ? null : Number(item.duration_seconds),
    })
