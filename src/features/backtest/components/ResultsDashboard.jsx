...
export function ResultsDashboard({ workspace }) {
  const { results, loadingResults, selectedRun, comparisonData, bestRun, selectedMetrics, setError } = workspace
      ...
      {results && (
        <>
          <section className="metrics-grid">
            <MetricCard label="Analysis result" value={comparisonData.length ? "Available" : "—"} note="Historical evaluation" />
            <MetricCard label="Best excess return" value={bestRun ? percent(bestRun.excess_return) : '—'} note="Compared with Buy & Hold" />
            <MetricCard label="Best ending capital" value={bestRun ? money(bestRun.ending_capital) : '—'} note="Evaluation result" />
            <MetricCard label="Export package" value={exportingPackage ? "Preparing…" : "ZIP + CSV"} note="Performance, trades and diagnostics" onClick={exportPackage} disabled={exportingPackage} />
          </section>

          <section className="panel chart-panel">
            <div className="section-heading compact"><div><span className="section-kicker">Performance comparison</span><h2>Portfolio vs Buy & Hold</h2></div><button className="button ghost" type="button" onClick={exportComparison} disabled={exportingComparison}>{exportingComparison ? "Preparing…" : "Export CSV"}</button></div>
            <div className="chart large-chart">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={comparisonData} margin={{ top: 12, right: 18, left: 0, bottom: 44 }}>
                  ...
                  <Legend />
                  <Bar dataKey="portfolioPct" name="Portfolio" fill="var(--series-1)" radius={[5, 5, 0, 0]} />
          ...
          <section className="panel comparison-table-panel">
            <div className="section-heading compact"><div><span className="section-kicker">Evaluation comparison</span><h2>Performance metrics</h2></div></div>
            <div className="table-scroll">
              <table>
                <thead><tr><th>Result</th><th>Folds</th><th>Capital</th><th>Return</th><th>Buy & Hold</th><th>Excess</th><th>CAGR</th><th>Sharpe</th><th>Max DD</th><th>Rotations</th><th>Avg hold</th></tr></thead>
                <tbody>
                  {comparisonData.map((row) => (
                    <tr key={row.key}>
                      <td><strong>{row.label}</strong></td>
                      <td>{number(row.fold_count, 0)}</td>
                      <td>{money(row.ending_capital)}</td>
                      <td>{percent(row.return)}</td>
                      <td>{percent(row.benchmark_return)}</td>
                      <td className={Number(row.excess_return) >= 0 ? 'positive' : 'negative'}>{percent(row.excess_return)}</td>
                      <td>{percent(row.cagr)}</td>
                      <td>{number(row.sharpe, 3)}</td>
                      <td>{percent(row.maximum_drawdown)}</td>
                      <td>{number(row.rotations, 0)}</td>
          ...
          {selectedRun && (
            <section className="panel detail-panel">
              <div className="section-heading compact detailed-run-heading">
                <div className="detailed-run-title"><span className="section-kicker">Detailed result</span><h2>{selectedRun.label}</h2></div>
              </div>
              <section className="metrics-grid compact-metrics">
                <MetricCard label="Final capital" value={money(selectedMetrics.ending_capital)} note={`Initial ${money(selectedMetrics.initial_capital)}`} />
                <MetricCard label="CAGR" value={percent(selectedMetrics.cagr)} note={`Sharpe ${number(selectedMetrics.sharpe, 3)}`} />
                <MetricCard label="Max drawdown" value={percent(selectedMetrics.maximum_drawdown)} note={`Exposure ${percent(selectedMetrics.exposure)}`} />
                <MetricCard label="Rotations" value={number(selectedMetrics.rotations, 0)} note={`${number(selectedMetrics.average_holding_days, 2)} average holding days`} />
              </section>
                ...
                <div className="chart large-chart">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={selectedRun.series} margin={{ top: 12, right: 18, left: 0, bottom: 20 }}>
                      ...
                      <Legend />
                      <Line dataKey="portfolioEquity" name="Portfolio" dot={false} stroke="var(--series-1)" strokeWidth={2} />
                      <Line dataKey="benchmarkEquity" name="Buy & Hold" dot={false} stroke="var(--series-2)" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
              ...
              <article className="trades-panel">
                <div className="trade-table-heading">
                  <div>
                    ...
                    <h3>Decision log</h3>
                    <p>BUY, SELL and HOLD records for the selected result.</p>
                  </div>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th>Action</th><th>Asset</th><th>Price</th><th>Quantity</th><th>Capital</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      }) : <tr><td className="empty-cell" colSpan="6">No decisions match the selected action filters.</td></tr>}
                    </tbody>
                  </table>
                </div>
