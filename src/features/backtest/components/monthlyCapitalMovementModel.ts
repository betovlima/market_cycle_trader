...
export function movementTimestamp(value: any) {
  const timestamp = Date.parse(value || '')
  return Number.isFinite(timestamp) ? timestamp : null
}

export function normalizedMovementAsset(value: any) {
...
export function fullMonthName(year: number, month: number) {
  return new Intl.DateTimeFormat(getIntlLocale(), { month: 'long', year: 'numeric', timeZone: 'UTC' })
    .format(new Date(Date.UTC(year, month - 1, 1)))
}

export function compactMoney(value: any) {
...
export function monthMovementModel(rotations: any[], equity: any[]) {
  const movements: AppRecord[] = (rotations || [])
    .map((item: AppRecord) => ({ ...item, executedAtValue: movementTimestamp(item.executed_at) }))
    .filter((item: AppRecord) => item.executedAtValue !== null)
    .sort((left: any, right: any) => left.executedAtValue - right.executedAtValue)

  const equityRows: AppRecord[] = (equity || [])
    .map((item: AppRecord) => ({
      ...item,
      timestampValue: movementTimestamp(item.timestamp || item.recorded_at),
      equityValue: Number(item.simulation_equity),
    }))
    .filter((item: AppRecord) => item.timestampValue !== null && Number.isFinite(item.equityValue))
    .sort((left: any, right: any) => left.timestampValue - right.timestampValue)

  const months = new Map<string, AppRecord>()
  ...
  const topAsset = (counts: any) => [...counts.entries()].sort((left: any, right: any) => right[1] - left[1] || left[0].localeCompare(right[0]))[0]?.[0] || null
  let maxAbsPnl = 0
  let maxMovements = 0
  ...
  for (const month of months.values()) {
    ...
    month.averageHolding = month.holdingValues.length
      ? month.holdingValues.reduce((total: number, value: any) => total + value, 0) / month.holdingValues.length
      : null
  ...
  const years = [...new Set([...months.values()].map((item: AppRecord) => item.year))].sort((left: any, right: any) => left - right)
  return { months, years, maxAbsPnl, maxMovements, maxCashSessions, maxHolding }
}

export function heatmapMetric(month: AppRecord, mode: string, model: AppRecord) {
  if (mode === 'movements') {
...
export function aggregateHeatmapMetric(items: any[], mode: string, model: AppRecord) {
  const months = (items || []).filter(Boolean)
  if (!months.length) return { label: '—', tone: 'empty', alpha: .08 }
  if (mode === 'movements') {
    const value = months.reduce((total: number, month: AppRecord) => total + Number(month.movementCount || 0), 0)
    return { label: String(value), tone: 'movements', alpha: .42 }
  }
  if (mode === 'cash') {
    const value = months.reduce((total: number, month: AppRecord) => total + Number(month.cashSessions || 0), 0)
    return { label: `${value}d`, tone: 'cash', alpha: .42 }
  }
  if (mode === 'holding') {
    const values = months.flatMap((month: AppRecord) => month.holdingValues || []).map(Number).filter(Number.isFinite)
    if (!values.length) return { label: '—', tone: 'empty', alpha: .08 }
    const average = values.reduce((total: number, value: any) => total + value, 0) / values.length
    return { label: `${average.toFixed(1)}d`, tone: 'holding', alpha: .42 }
  }
  const total = months.reduce((sum: number, month: AppRecord) => sum + Number(month.totalRealizedPnl || 0), 0)
