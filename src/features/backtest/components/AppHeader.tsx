...
export function AppHeader({ activeTab, onTabChange, session, capabilities = {}, onLogout }: AppRecord) {
  const navItems = NAV_ITEMS.filter(({ capability }: AppRecord) => capabilities?.[capability] === true)
  return (
    <header className="app-header">
      ...
      <div className="header-right">
        ...
        <nav className="main-nav" aria-label={tr("Main navigation")}>
          {navItems.map(({ id, label, Icon }: AppRecord) => (
