...
export function BacktestTradeEventDot({ cx, cy, payload }: AppRecord) {
        ...
        return (
          <g key={event.markerKey} className={`backtest-trade-marker ${isBuy ? 'buy' : 'sell'}`} transform={`translate(0, ${markerY})`}>
            <circle
              ...
              onPointerDown={(pointerEvent: any) => pointerEvent.stopPropagation()}
...
export function BacktestChartTooltip({ active, payload }: AppRecord) {
...
export function MetricLabel({ id, label, hint, hintDetails = [] }: AppRecord) {
  return (
...
export function Metric({ id, label, value, note = null, tone = '', hint = '', hintDetails = [] }: AppRecord) {
  return (
...
export function SortableHeader({ label, field, sort, onSort, hint = '' }: AppRecord) {
...
export function FilterButton({ active, label, onClick, tone = '', children = null }: AppRecord) {
  return (
    <button
      type="button"
...
export function Pagination({ page, pages, total, pageSize, onPageChange }: AppRecord) {
