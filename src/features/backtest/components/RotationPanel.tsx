...
export function RotationPanel({ jobId, payload, loading, error }: AppRecord) {
  ...
  const realizedPnlBreakdown = useMemo(() => {
    const realized = rotations
      .map((item: AppRecord) => item.realized_pnl)
      .filter((value: any) => value !== null && value !== undefined && Number.isFinite(Number(value)))
      .map(Number)
    const grossProfits = realized.filter((value: any) => value > 0).reduce((total: number, value: any) => total + value, 0)
    const grossLosses = realized.filter((value: any) => value < 0).reduce((total: number, value: any) => total + value, 0)
    return {
      grossProfits,
      grossLosses,
      averageRealizedPnl: realized.length ? realized.reduce((total: number, value: any) => total + value, 0) / realized.length : null,
    }
  }, [rotations])

  const holdingStats = useMemo(() => {
    const values = rotations
      .map((item: AppRecord) => item.holding_days)
      .filter((value: any) => value !== null && value !== undefined && Number.isFinite(Number(value)))
      .map(Number)
  ...
  const filteredRows = useMemo(() => {
    ...
    const rows = rotations.filter((item: AppRecord) => {
      if (normalizedQuery) {
    ...
    return sortRows(rows, sort, {
      executed_at: (item: AppRecord) => Date.parse(item.executed_at || '') || 0,
      from_asset: (item: AppRecord) => String(item.from_asset || 'CASH'),
      to_asset: (item: AppRecord) => String(item.to_asset || 'CASH'),
      holding_days: (item: AppRecord) => item.holding_days == null ? null : Number(item.holding_days),
      position_return: (item: AppRecord) => item.position_return == null ? null : Number(item.position_return),
      realized_pnl: (item: AppRecord) => item.realized_pnl == null ? null : Number(item.realized_pnl),
      transaction_fees: (item: AppRecord) => item.transaction_fees == null ? null : Number(item.transaction_fees),
    })
    ...
    <section className="backtest-workspace-section rotation-workspace-section">
      ...
      <div className="table-wrap backtest-table-wrap">
        <table className="dashboard-table rotation-table backtest-sortable-table">
          <thead>
            <tr>
              <SortableHeader label={tr("Executed")} field="executed_at" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.executed_at} />
              <SortableHeader label={tr("Sold")} field="from_asset" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.from_asset} />
              <SortableHeader label={tr("Bought")} field="to_asset" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.to_asset} />
              <SortableHeader label={tr("Holding")} field="holding_days" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.holding_days} />
              <SortableHeader label={tr("Position Return")} field="position_return" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.position_return} />
              <SortableHeader label={tr("Realized P/L")} field="realized_pnl" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.realized_pnl} />
              <SortableHeader label={tr("Fees")} field="transaction_fees" sort={sort} onSort={(key: string) => setSort((current: any) => toggleSort(current, key))} hint={ROTATION_HINTS.transaction_fees} />
            </tr>
          </thead>
          <tbody>
            {paginatedRows.length ? paginatedRows.map((item: AppRecord, index: number) => (
