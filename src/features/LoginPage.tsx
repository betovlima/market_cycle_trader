...
let googleCredentialHandler: AppCallback | null = null
...
function keepInvitationAndRemoveToken(invitationId: string) {
...
function GoogleIdentityButton({ disabled, onCredential, onError }: AppRecord) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      try {
        googleCredentialHandler = (response: AppRecord) => {
        ...
        if (!googleIdentityInitialized) {
          googleIdentity.initialize({
            ...
            callback: (response: AppRecord) => googleCredentialHandler?.(response),
            auto_select: false,
...
export function LoginPage({ onAuthenticated }: AppRecord) {
  const initialAccess = useRef(accessFromLocation())
  const [locator, setLocator] = useState(initialAccess.current)
  const [preview, setPreview] = useState<AppRecord | null>(null)
  ...
  async function loadPreview(nextLocator: AppRecord) {
    ...
    setError('')
    try {
      const body: AppRecord = { invitation_id: nextLocator.invitation_id }
  ...
  async function authenticateGoogle(credential: string) {
    ...
    setError('')
    try {
      const body: AppRecord = { credential }
        ...
        {preview ? (
          <div className="verified-access-panel">
            ...
            <dl className="verified-access-details">
              ...
              <div><dt>{tr("Invitation status")}</dt><dd>{tr(preview.status.replaceAll('_', ' ').replace(/\b\w/g, (letter: any) => letter.toUpperCase()))}</dd></div>
