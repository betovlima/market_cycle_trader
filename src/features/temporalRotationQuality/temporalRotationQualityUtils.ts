...
export function toInput(value: any) {
...
export function numberListText(values: any[]) {
  return Array.isArray(values) ? values.join(', ') : ''
}

export function parseNumberList(value: any) {
  return String(value || '')
    .split(/[\s,;]+/)
...
export function manualCandidateText(values: any[]) {
  if (!Array.isArray(values)) return ''
  return values.map((item: AppRecord) => [item.drawdown_trigger, item.rotation_score_tolerance, item.challenger_quality_floor].filter((value: any) => value !== null && value !== undefined).join(', ')).join('\n')
}

export function parseManualCandidates(value: any) {
  return String(value || '')
    .split(/\r?\n/)
    ...
    .filter((item: AppRecord) => Number.isFinite(item.drawdown_trigger) && Number.isFinite(item.rotation_score_tolerance))
}
...
export function preferredDiagnosticCandidateId(evidence: any) {
  ...
  candidates.sort((a: any, b: any) => Number(b.ending_capital || 0) - Number(a.ending_capital || 0))
  return candidates[0]?.candidate_id || ''
}

export function evidenceWorkflowState(item: AppRecord) {
  if (!item) return '—'
