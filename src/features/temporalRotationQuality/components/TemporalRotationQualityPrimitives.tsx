...
export function EvidenceBadge({ passed }: AppRecord) {
...
export function NumericField({ label, value, onChange, disabled, min = undefined, max = undefined, step = 'any' }: AppRecord) {
  return (
    <label className="rotation-quality-field">
      <span>{tr(label)}</span>
      <input type="number" value={value} min={min} max={max} step={step} disabled={disabled} onChange={(event: any) => onChange(event.target.value)} />
    </label>
  )
}

export function Metric({ label, value, tone = '' }: AppRecord) {
  return <div className={`rotation-quality-metric ${tone}`}><span>{tr(label)}</span><strong>{value}</strong></div>
}

export function WorkflowStep({ label, state, tone = '' }: AppRecord) {
  return (
...
export function ResearchResult({ research }: AppRecord) {
...
export function EvidenceResult({ validation }: AppRecord) {
    ...
    <div className="rotation-quality-evidence-result">
      ...
      <div className="rotation-quality-table-shell">
        <table className="rotation-quality-table">
          ...
          <tbody>
            {validation.candidates.map((candidate: AppRecord) => (
...
export function DiagnosticResult({ diagnostic }: AppRecord) {
    ...
    <div className="rotation-quality-diagnostic-result">
      ...
      {folds.length ? <div className="rotation-quality-table-shell">
        <table className="rotation-quality-table">
          ...
          <tbody>{folds.map((item: AppRecord) => <tr key={item.fold_id}><td>{item.fold_id}</td><td>{item.blocked_rotations}</td><td>{item.helpful}</td><td>{item.harmful}</td><td>{item.neutral}</td><td className={Number(item.immediate_net_rotation_benefit_dollars) >= 0 ? 'positive' : 'negative'}>{money(item.immediate_net_rotation_benefit_dollars)}</td></tr>)}</tbody>
        </table>
      </div> : null}
      {rows.length ? <div className="rotation-quality-table-shell">
        <table className="rotation-quality-table diagnostic-features">
          ...
          <tbody>{rows.map((item: AppRecord) => <tr key={item.engineered_metric}><td>{item.feature}</td><td>{item.engineered_metric}</td><td>{number(item.helpful_mean, 5)}</td><td>{number(item.harmful_mean, 5)}</td><td>{number(item.standardized_separation, 4)}</td><td>{tr(item.helpful_direction)}</td></tr>)}</tbody>
        </table>
      </div> : null}
    </div>
  )
