...
export function AdminFieldLabel({ id, label, hint }: AppRecord) {
  return (
...
export function AdminSortableTh({ label, field, sort, onSort, hint = '' }: AppRecord) {
...
export function AdminListToolbar({ query, onQueryChange, placeholder, count, children }: AppRecord) {
  return (
    <div className="admin-list-toolbar">
      <label className="admin-list-search">
        <SearchIcon size={15} />
        <input type="search" value={query} onChange={(event: any) => onQueryChange(event.target.value)} placeholder={tr(placeholder)} aria-label={tr(placeholder)} />
      </label>
...
export function AdminPagination({ page, pages, total, pageSize, onPageChange }: AppRecord) {
...
export function AccessLinkDialog({ access, onClose, onError }: AppRecord) {
  ...
  return (
    <div className="access-link-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="access-link-dialog" role="dialog" aria-modal="true" aria-labelledby="access-link-title" onMouseDown={(event: any) => event.stopPropagation()}>
...
export function AdminSummary({ icon, label, value, tone = '', hint = '' }: AppRecord) {
