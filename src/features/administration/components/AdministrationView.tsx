...
export function AdministrationView({ workspace }: AppRecord) {
  const {
    busyId,
    counts,
    createInvitation,
          ...
          <form className="admin-invite-form identity-invite-form admin-workspace-invite-form" onSubmit={createInvitation}>
            <label>
              ...
              <input
                ...
                onChange={(event: any) => setForm({ ...form, guest_name: event.target.value })}
                maxLength={120}
                required
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event: any) => setForm({ ...form, authorized_email: event.target.value })}
                maxLength={254}
                autoComplete="off"
                required
              />
            ...
            <label>
              ...
              <select
                ...
                onChange={(event: any) => setForm({ ...form, duration_seconds: event.target.value })}
                required
              >
                {DURATION_OPTIONS.map(([value, label]: any[]) => (
            ...
            <label>
              ...
              <select
                ...
                onChange={(event: any) => setForm({ ...form, max_active_sessions: event.target.value })}
                required
              >
                {SESSION_OPTIONS.map((value: any) => <option key={value} value={value}>{value}</option>)}
              </select>
            </label>
            <button
              type="submit"
        ...
        <section className="admin-workspace-section admin-access-section">
          ...
          {loading ? (
            ...
            <>
              ...
              >
                <select value={invitationStatus} onChange={(event: any) => setInvitationStatus(event.target.value)} aria-label={tr("Filter invitation status")}>
                ...
                <select value={invitationRole} onChange={(event: any) => setInvitationRole(event.target.value)} aria-label={tr("Filter invitation role")}>
              ...
              <div className="admin-workspace-table-wrap">
                <table className="market-table admin-table identity-admin-table admin-sortable-table">
                  <thead>
                    <tr>
                      <AdminSortableTh label={tr("User")} field="guest_name" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.guestName} />
                      <AdminSortableTh label={tr("Role")} field="role" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.role} />
                      <AdminSortableTh label={tr("Status")} field="status" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.status} />
                      <AdminSortableTh label={tr("Sessions")} field="sessions" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.sessions} />
                      <AdminSortableTh label={tr("Claimed identity")} field="claimed_identity" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.claimedIdentity} />
                      <AdminSortableTh label={tr("Expires")} field="expires_at" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.expires} />
                      <AdminSortableTh label={tr("Last access")} field="last_access_at" sort={invitationSort} onSort={(key: string) => setInvitationSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.lastAccess} />
                      <th>{tr("Actions")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    ) : visibleInvitations.map((item: AppRecord) => {
                          ...
                          <td data-label={tr('Sessions')}>
                            <div className="session-limit-control">
                              ...
                              <select
                                ...
                                onChange={(event: any) => setSessionLimits({ ...sessionLimits, [item.id]: event.target.value })}
                                disabled={locked}
                          ...
                          <td data-label={tr('Actions')}>
                            <div className="admin-row-actions identity-row-actions">
                              ...
                              <select
                                ...
                                onChange={(event: any) => setExtendDurations({ ...extendDurations, [item.id]: event.target.value })}
                                disabled={locked || primaryAdministrator}
                                aria-label={`Duration for ${item.guest_name}`}
                              >
                                {DURATION_OPTIONS.map(([value, label]: any[]) => <option key={value} value={value}>+{tr(label)}</option>)}
                              </select>
            ...
            <>
              ...
              >
                ...
                <select value={logRole} onChange={(event: any) => setLogRole(event.target.value)} aria-label={tr("Filter audit role")}>
                  <option value="all">{tr("All roles")}</option>
                  <option value="viewer">{tr("Viewer")}</option>
                  <option value="trader">{tr("Trader")}</option>
                  <option value="admin">{tr("Administrator")}</option>
              ...
              <div className="admin-workspace-table-wrap">
                <table className="market-table access-log-table admin-sortable-table">
                  <thead>
                    <tr>
                      <AdminSortableTh label={tr("Time")} field="created_at" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditTime} />
                      <AdminSortableTh label={tr("Event")} field="event" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditEvent} />
                      <AdminSortableTh label={tr("User")} field="guest_name" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditUser} />
                      <AdminSortableTh label={tr("Google identity")} field="identity_email" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditIdentity} />
                      <AdminSortableTh label={tr("Role")} field="role" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditRole} />
                      <AdminSortableTh label={tr("Result")} field="success" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditResult} />
                      <AdminSortableTh label={tr("Client")} field="client_ip" sort={logSort} onSort={(key: string) => setLogSort((current: any) => toggledSort(current, key))} hint={ADMIN_HINTS.auditClient} />
                    </tr>
                  </thead>
                  <tbody>
                    {visibleLogs.length === 0 ? (
                      <tr><td colSpan="7" className="empty-table-cell">{tr("No access events match the selected filters.")}</td></tr>
                    ) : visibleLogs.map((item: AppRecord) => (
                      <tr key={item.id}>
