...
export function defaultSessions(role: string) {
...
export function sortedRows(rows: any[], sort: AppRecord, valueGetter: AppCallback | null = null) {
...
export function toggledSort(current: any, key: string) {
  if (current.key === key) return { key, direction: current.direction === 'asc' ? 'desc' : 'asc' }
  return { key, direction: 'asc' }
}

export function boundedPage(page: number, total: number, pageSize: number) {
