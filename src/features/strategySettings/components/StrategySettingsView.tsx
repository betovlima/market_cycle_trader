...
export function StrategySettingsView({ workspace }: AppRecord) {
  const {
    activeJob,
    busy,
    canMarkCandidate,
            ...
            <label className="strategy-parameter-search">
              ...
              <div className="strategy-parameter-search-control">
                <input
                  ...
                  onChange={(event: any) => setParameterSearch(event.target.value)}
                  autoComplete="off"
                />
          ...
          <form className="strategy-parameter-form" onSubmit={saveStrategy}>
            <div className="strategy-metadata-grid">
              <label>
                ...
                <input value={name} onChange={(event: any) => setName(event.target.value)} disabled={selected.locked} required />
              </label>
              <label>
                <StrategyFieldLabel id="hint-strategy-description" label={tr("Description")} hint={STRATEGY_FIELD_HINTS.description} align="right" />
                <input value={description} onChange={(event: any) => setDescription(event.target.value)} disabled={selected.locked} />
              </label>
            </div>

            <div className="strategy-parameter-groups">
              {groupedParameters.map((group: any, index: number) => (
                <details key={`${group.id}:${parameterSearch ? 'filtered' : 'all'}`} open={parameterSearch ? true : index === 0 || group.id === 'model'}>
                  ...
                  <div className="strategy-parameter-grid">
                    {group.fields.map((field: string, fieldIndex: number) => (
                      <ParameterField
                        key={field}
                        name={field}
            ...
            {!selected.locked ? (
              <div className="strategy-save-row">
                <label>
                  ...
                  <input value={changeNote} onChange={(event: any) => setChangeNote(event.target.value)} maxLength={500} placeholder={tr('Optional audit note')} />
                </label>
