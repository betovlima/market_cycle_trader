...
export function StrategyFieldLabel({ id, label, hint, align = 'left' }: AppRecord) {
  return (
...
export const ParameterField = memo(function ParameterField({ name, value, reference, schema, hintAlign = 'left', disabled, onChange }: AppRecord) {
  ...
  if (enumValues.length) {
    return (
      <label>
        {fieldHeading}
        <select value={value ?? ''} disabled={disabled} onChange={(event: any) => onChange(name, event.target.value)}>
          {enumValues.map((option: any) => <option key={String(option)} value={option}>{String(option)}</option>)}
        </select>
      </label>
    )
  }
  if (typeof reference === 'boolean') {
    return (
      <label className="strategy-boolean-field">
        {fieldHeading}
        <input type="checkbox" checked={Boolean(value)} disabled={disabled} onChange={(event: any) => onChange(name, event.target.checked)} />
      </label>
    )
  }
  ...
  if (Array.isArray(reference)) {
    return (
      <label className="strategy-array-field">
        {fieldHeading}
        <textarea value={value ?? ''} disabled={disabled} rows="2" spellCheck="false" onChange={(event: any) => onChange(name, event.target.value)} />
      </label>
    )
  }
  ...
  return (
    <label>
      {fieldHeading}
      <input value={value ?? ''} disabled={disabled} onChange={(event: any) => onChange(name, event.target.value)} />
    </label>
  )
})
