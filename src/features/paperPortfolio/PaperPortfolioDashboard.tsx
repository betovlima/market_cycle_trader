...
export function PaperPortfolioDashboard() {
  ...
  const loadRobotStatus = useCallback(async ({ silent = false }: AppRecord = {}) => {
    try {
      const response = await apiFetch(`${API}/paper-market/public-robot-status`)
      if (mountedRef.current) setRobot(response)
    } catch {
      if (mountedRef.current) {
        setRobot((current: any) => current ? { ...current, scheduler_alive: false, status: 'unavailable' } : { enabled: false, scheduler_alive: false, status: 'unavailable' })
      }
    }
  }, [])

  const loadPortfolio = useCallback(async ({ silent = false }: AppRecord = {}) => {
  ...
  const refreshPortfolio = useCallback(async ({ silent = false, includeRobot = false }: AppRecord = {}) => {
    if (portfolioTimerRef.current) window.clearTimeout(portfolioTimerRef.current)
          ...
          <section className="portfolio-orders-section">
            ...
            <div className="table-wrap portfolio-orders-table-wrap compact-order-scroll">
              <table className="dashboard-table portfolio-orders-table">
                ...
                <tbody>
                  {data.recent_orders?.length ? data.recent_orders.map((order: any, index: number) => (
                    <tr key={`${order.created_at || 'order'}-${order.symbol || 'asset'}-${index}`}>
                      ...
                      <td>{order.status ? tr(String(order.status).replaceAll('_', ' ').replace(/\b\w/g, (letter: any) => letter.toUpperCase())) : '—'}</td><td>{order.filled_quantity ?? order.quantity ?? '—'}</td><td>{order.filled_average_price ? money(order.filled_average_price) : '—'}</td>
                    </tr>
