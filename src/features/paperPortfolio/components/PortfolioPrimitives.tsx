...
export function TradingSessionStrip({ connection, marketClock, robot, now, refreshing, nextRefreshAt }: AppRecord) {
  ...
  return (
    <div className="portfolio-session-strip" aria-label={tr("Trading session status")} aria-live="polite">
      <div className="portfolio-session-main">
        <div className="portfolio-session-title" title={tr('Robot phase: {phase}. Next execution: {session}. {broker}.', { phase: tr(phase.replace(/\b\w/g, (letter: any) => letter.toUpperCase())), session, broker: checkedLabel })}>
      ...
      <div className="portfolio-session-schedule" aria-label={tr('Robot phase {phase}. Next execution {session}.', { phase: tr(phase.replace(/\b\w/g, (letter: any) => letter.toUpperCase())), session })}>
        {schedule.map((item: AppRecord) => (
...
export function PortfolioMetric({ label, value, detail = null, tone = '' }: AppRecord) {
  return (
...
export function PortfolioMetricsStrip({ data, position }: AppRecord) {
...
export function CurrentPosition({ position, cash }: AppRecord) {
  if (!position) {
    return (
...
export function TradeEventDot({ cx, cy, payload }: AppRecord) {
...
export function PortfolioChartTooltip({ active, payload }: AppRecord) {
  if (!active || !payload?.length) return null
  const pointPayload = payload.find((item: AppRecord) => item?.payload?.portfolio_value !== undefined)?.payload
