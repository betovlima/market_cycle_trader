...
function valuesFromModel(model: AppRecord) {
  return Object.fromEntries((model?.fields || []).map((field: AppRecord) => [field.name, field.value]))
}
...
function inputValue(field: AppRecord, value: any) {
  if (field.type === 'boolean') return Boolean(value)
  if (value === null || value === undefined) return ''
  return String(value)
}

function normalizedValue(field: AppRecord, value: any) {
  if (field.type === 'boolean') return Boolean(value)
  if (field.type === 'integer') return Number.parseInt(value, 10)
  return Number(value)
}

function ModelField({ field, value, disabled, onChange }: AppRecord) {
  const hint = {
...
export function ModelResearchSettingsPanel({
  ...
  const [formValues, setFormValues] = useState<AppRecord>({})
  ...
  const initializeFromPayload = useCallback((nextPayload: AppRecord, family: string = '') => {
    setPayload(nextPayload)
    const models = nextPayload?.models || []
    const targetId = family && models.some((item: AppRecord) => item.id === family)
      ? family
      : savedModelFamily && models.some((item: AppRecord) => item.id === savedModelFamily)
        ? savedModelFamily
        : models[0]?.id || ''
    setSelectedModelId(targetId)
    const target = models.find((item: AppRecord) => item.id === targetId)
  ...
  const selectedModel = useMemo(
    () => (payload?.models || []).find((item: AppRecord) => item.id === selectedModelId) || null,
  ...
  const visibleModelFields = useMemo(() => {
    ...
    return fields.filter((field: AppRecord) => {
  ...
  function selectModel(modelId: string) {
    ...
    const model = (payload?.models || []).find((item: AppRecord) => item.id === modelId)
  ...
  function changeField(field: AppRecord, value: any) {
    setFormValues((current: any) => ({ ...current, [field.name]: normalizedValue(field, value) }))
  }
    ...
    <Container className={`${embedded ? 'model-research-embedded' : 'settings-workspace-section'} model-research-settings-section`}>
      ...
      <div className="model-research-selector-row model-research-selector-single">
        <label>
          ...
          <select value={selectedModelId} onChange={(event: any) => selectModel(event.target.value)} disabled={readOnly || Boolean(strategy?.locked)}>
            {(payload.models || []).map((model: AppRecord) => <option key={model.id} value={model.id}>{model.label}</option>)}
          </select>
        </label>
      ...
      {selectedModel ? (
        <form onSubmit={save} className="model-research-form">
          <div className="model-research-fields-grid">
            {visibleModelFields.map((field: AppRecord) => (
              <ModelField key={field.name} field={field} value={formValues[field.name]} disabled={readOnly || Boolean(strategy?.locked)} onChange={(value: any) => changeField(field, value)} />
            ))}
          </div>
          {!readOnly && !strategy?.locked ? (
            <div className="model-research-save-row">
              <label>
                ...
                <input value={reason} onChange={(event: any) => setReason(event.target.value)} maxLength={500} required placeholder={tr('Describe why this Strategy model is changing')} />
              </label>
