...
export function TemporalIntelligencePanel({ capabilities = {}, onSessionExpired, tuningStrategy = null, studyProcessing = null }: AppRecord) {
  ...
  const load = useCallback(async ({ silent = false }: AppRecord = {}) => {
  ...
  useEffect(() => {
    ...
    if (!horizonMetrics.some((item: AppRecord) => Number(item.horizon) === Number(selectedHorizon))) {
      setSelectedHorizon(Number(horizonMetrics[0].horizon))
    }
  }, [horizonMetrics, selectedHorizon])

  const selectedMetrics = useMemo(() => horizonMetrics.find((item: AppRecord) => Number(item.horizon) === Number(selectedHorizon)) || null, [horizonMetrics, selectedHorizon])
  const forecasts = useMemo(() => (result?.latest_forecasts || []).filter((item: AppRecord) => Number(item.horizon) === Number(selectedHorizon)), [result?.latest_forecasts, selectedHorizon])
  ...
  const multiHorizonBestForecast = multiHorizonForecasts.find((item: AppRecord) => item.shadow_target) || null
  ...
  async function materializeStrategy() {
    ...
    try {
      ...
      if (strategy) {
        setRun((current: any) => current ? { ...current, materialized_strategy_id: strategy.id, materialized_strategy_name: strategy.name } : current)
  ...
  const bestForecast = decisionExperiment ? forecasts.find((item: AppRecord) => item.shadow_target) : null
  ...
  return (
    <div className="temporal-intelligence-panel">
      ...
      {result ? <>
        ...
        {multiHorizonPolicy && multiHorizonMetrics ? <>
          <section className="temporal-section">
            ...
            <div className="temporal-run-meta temporal-multi-horizon-roles">
              <span>{tr('Entry Horizons')}</span><strong>{(multiHorizonMetrics.entry_horizons || []).map((item: AppRecord) => `${item}d`).join(' · ')}</strong><i>·</i>
              <span>{tr('Hold Horizons')}</span><strong>{(multiHorizonMetrics.hold_horizons || []).map((item: AppRecord) => `${item}d`).join(' · ')}</strong><i>·</i>
              <span>{tr('Risk Horizons')}</span><strong>{(multiHorizonMetrics.risk_horizons || []).map((item: AppRecord) => `${item}d`).join(' · ')}</strong>
            </div>
          </section>
        </> : null}
        ...
        {!decisionExperiment ? <section className="temporal-section"><div className="temporal-section-heading"><h3>{tr('Latest Shadow Forecast')}</h3><div className="temporal-horizon-buttons">{horizonMetrics.map((item: AppRecord) => <button type="button" key={item.horizon} className={Number(selectedHorizon) === Number(item.horizon) ? 'active' : ''} onClick={() => setSelectedHorizon(Number(item.horizon))}>{item.horizon}d</button>)}</div></div><LegacyForecastTable items={forecasts} /></section> : null}
      </> : null}
