...
function parseHours(value: any) {
  const hours = String(value || '')
    .split(',')
    .map((item: string) => Number(item.trim()))
    .filter((item: number) => Number.isInteger(item) && item >= 0 && item <= 23)
  return [...new Set(hours)].sort((left: any, right: any) => left - right)
}

export function DiscoverySettings({ settings, busy, onSave }: AppRecord) {
  ...
  return <form className="asset-discovery-settings" onSubmit={submit}>
    ...
    <div className="asset-discovery-settings-grid">
      <label className="asset-discovery-toggle-field">
        ...
        <input type="checkbox" checked={automaticEnabled} onChange={(event: any) => setAutomaticEnabled(event.target.checked)} />
      </label>
      ...
      <label>
        ...
        <input type="number" min="1" max="50" value={batchSize} onChange={(event: any) => setBatchSize(event.target.value)} />
      </label>
      ...
      <label>
        ...
        <input value={hours} onChange={(event: any) => setHours(event.target.value)} placeholder="18, 20, 22" />
      </label>
      ...
      <label>
        ...
        <input type="number" min="1" max="365" value={recheckDays} onChange={(event: any) => setRecheckDays(event.target.value)} />
      </label>

      <label className="asset-discovery-reason-field">
        <span>{tr('Change reason')}</span>
        <input value={reason} onChange={(event: any) => setReason(event.target.value)} placeholder={tr('Optional audit note')} />
      </label>
