...
function dollarVolume(value: any) {
...
function historyLabel(item: AppRecord) {
...
export function CandidateTable({ candidates }: AppRecord) {
  ...
  const filtered = useMemo(() => {
    ...
    return candidates.filter((item: AppRecord) => {
  ...
  return <section className="asset-discovery-candidates">
    ...
    <div className="asset-discovery-table-toolbar">
      <label className="asset-discovery-search"><SearchIcon size={16} /><input value={query} onChange={(event: any) => { setQuery(event.target.value); setPage(1) }} placeholder={tr('Filter symbol')} /></label>
      <select value={status} onChange={(event: any) => { setStatus(event.target.value); setPage(1) }}>
    ...
    <div className="table-scroll asset-discovery-table-wrap">
      <table className="data-table asset-discovery-table">
        ...
        <tbody>
          {rows.length ? rows.map((item: AppRecord) => <tr key={item.symbol}>
            ...
            <td className="asset-discovery-reasons">
              {[...(item.reason_codes || []).map((code: any) => tr(String(code).replaceAll('_', ' '))), item.last_error].filter(Boolean).join(' · ') || '—'}
            </td>
    ...
    <div className="asset-discovery-pagination">
      ...
      <div><button type="button" className="secondary-action compact asset-discovery-action" disabled={safePage <= 1} onClick={() => setPage((value: any) => Math.max(1, value - 1))}>{tr('Previous')}</button><button type="button" className="secondary-action compact asset-discovery-action" disabled={safePage >= pages} onClick={() => setPage((value: any) => Math.min(pages, value + 1))}>{tr('Next')}</button></div>
    </div>
  </section>
}
