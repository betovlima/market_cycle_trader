...
export function dashboardAxisLabel(value: any, visibleSpan: any) {
...
export function dashboardSortValue(item: AppRecord, key: string) {
...
export function statusMatchesFilter(status: string, filter: string) {
...
export function tradeTone(value: any) {
  const number = Number(value)
  if (!Number.isFinite(number) || number === 0) return 'flat'
  return number > 0 ? 'profit' : 'loss'
}

export function decimal(value: any, digits: any = 4) {
  const number = Number(value)
  return Number.isFinite(number) ? number.toFixed(digits) : '—'
}

export function strategyValue(value: any) {
...
export function buildCashIntervals(rows: any[]) {
  const intervals: AppRecord[] = []
