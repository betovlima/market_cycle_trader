...
function completedRows(rows: any[]) {
  return (Array.isArray(rows) ? rows : [])
    .filter((item: AppRecord) => String(item?.status || '').toLowerCase() === 'completed')
    .sort((left: any, right: any) => String(right?.created_at || right?.finished_at || '').localeCompare(String(left?.created_at || left?.finished_at || '')))
}

function optionLabel(item: AppRecord) {
...
export function DashboardBacktestAnalyticsSection({ fallbackJobs = [], initialProcessingId = "" }: AppRecord) {
  ...
  const history = useMemo(() => {
    ...
    return primary.length ? primary : completedRows(fallbackJobs).map((item: AppRecord) => ({ ...item, processing_kind: 'backtest', processing_label: tr('Backtest') }))
  }, [fallbackJobs, jobs])
  ...
  useEffect(() => {
    ...
    if (requested && history.some((item: AppRecord) => String(item.id) === requested) && String(jobId) !== requested) {
      setJobId(requested)
      return
    }
    if (!jobId || !history.some((item: AppRecord) => String(item.id) === String(jobId))) {
    ...
    <section className="dashboard-analytics-hub">
      <div className="dashboard-analytics-history-head">
        ...
        <label className="dashboard-analytics-run-select">
          ...
          <select value={jobId} onChange={(event: any) => setJobId(event.target.value)} disabled={!history.length || loading}>
            {history.map((item: AppRecord) => <option key={item.id} value={item.id}>{optionLabel(item)}</option>)}
          </select>
