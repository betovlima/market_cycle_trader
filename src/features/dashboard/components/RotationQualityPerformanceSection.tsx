...
function kindLabel(value: any) {
  if (value === 'certification') return tr('Certification')
  if (value === 'validation') return tr('Validation')
  return tr('Research')
}

function candidateRows(execution: AppRecord) {
  return [...(execution?.candidates || [])].sort((a: any, b: any) => Number(b?.candidate_metrics?.ending_capital || 0) - Number(a?.candidate_metrics?.ending_capital || 0))
}

function experimentSequence(candidateId: string, fallbackIndex: number) {
  const match = String(candidateId || '').match(/(\d+)(?!.*\d)/)
  return match ? Number(match[1]) : fallbackIndex + 1
}

function normalizedExperimentRows(rows: any[], kind: string) {
  return (Array.isArray(rows) ? rows : [])
    .filter((item: AppRecord) => String(item?.candidate_id || '').toUpperCase() !== 'CONTROL')
    .map((item: AppRecord, index: number) => {
    ...
    .sort((left: any, right: any) => left.sequence - right.sequence || String(left.candidate_id).localeCompare(String(right.candidate_id)))
}

function SummaryMetric({ label, value, tone = '' }: AppRecord) {
  return <div className={`rq-dashboard-metric ${tone}`}><span>{tr(label)}</span><strong>{value}</strong></div>
}

function ExperimentTooltip({ active = false, payload = [] }: AppRecord) {
  if (!active || !payload?.length) return null
  const point = payload.find((item: AppRecord) => item?.payload?.candidate_id)?.payload
...
function ExperimentResultsChart({ rows, controlCapital, filter, onFilterChange, loading }: AppRecord) {
  const visibleRows = useMemo(() => rows.filter((row: AppRecord) => filter === 'all' || row.status === filter), [filter, rows])
  const values = visibleRows.map((row: AppRecord) => Number(row.ending_capital)).filter(Number.isFinite)
  ...
  return <article className="rq-dashboard-experiment-card">
    <div className="rq-dashboard-chart-head">
      ...
      <div className="rotation-monthly-heatmap-modes rq-experiment-filter" role="group" aria-label={tr('Experiment result filter')}>
        {[['pass', 'PASS'], ['fail', 'FAIL'], ['all', 'PASS & FAIL']].map(([key, label]: any[]) => <button key={key} type="button" className={filter === key ? 'active' : ''} onClick={() => onFilterChange(key)}>{label}</button>)}
      </div>
    </div>
    ...
    {loading ? <div className="backtest-loading-row">{tr('Loading experiments…')}</div> : visibleRows.length ? <div className="rq-dashboard-experiment-chart">
      <AnalyticsResponsiveContainer fallbackHeight={300}>
        <LineChart data={visibleRows} margin={{ top: 12, right: 18, bottom: 8, left: 12 }}>
          ...
          <YAxis domain={domain} tickFormatter={(value: any) => `$${Math.round(Number(value) / 1000)}k`} width={62} />
...
function RotationQualityMonthDialog({ month, processingId, candidateId, onClose }: AppRecord) {
  ...
  return createPortal(
    <div className="monthly-return-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="monthly-return-dialog rq-month-dialog" role="dialog" aria-modal="true" onMouseDown={(event: any) => event.stopPropagation()}>
...
export function RotationQualityPerformanceSection() {
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/temporal-rotation-quality-research/analytics/processings?limit=50`)
      .then((payload: AppRecord) => {
        if (!active) return
        const items = Array.isArray(payload?.items) ? payload.items : []
        setExecutions(items)
        if (items.length) setProcessingId((current: any) => current || String(items[0].id || ''))
      })
      .catch((requestError: ErrorLike) => { if (active) setError(requestError.message || tr('Unable to load Rotation Quality analytics.')) })
    return () => { active = false }
  }, [])

  const execution = useMemo(() => executions.find((item: AppRecord) => String(item.id) === String(processingId)) || null, [executions, processingId])
  ...
  useEffect(() => {
    ...
    if (!availableCandidates.some((item: AppRecord) => String(item.candidate_id) === String(candidateId))) {
  ...
  useEffect(() => {
    ...
    const request = kind === 'research'
      ? apiFetch(`${API}/temporal-rotation-quality-research/${encodeURIComponent(execution.research_id)}/candidates?limit=2000`)
          .then((payload: AppRecord) => payload?.items || [])
      : apiFetch(`${API}/temporal-rotation-quality-research/${encodeURIComponent(execution.research_id)}/validations/${encodeURIComponent(processingId)}`)
          .then((payload: AppRecord) => payload?.candidates || [])
    request
      .then((rows: any[]) => { if (active) setExperimentRows(normalizedExperimentRows(rows, kind)) })
      .catch((requestError: ErrorLike) => {
        if (!active) return
        setExperimentRows([])
  ...
  useEffect(() => {
    ...
    apiFetch(`${API}/temporal-rotation-quality-research/analytics/processings/${encodeURIComponent(processingId)}?candidate_id=${encodeURIComponent(candidateId)}`)
      .then((payload: AppRecord) => { if (active) setData(payload) })
      .catch((requestError: ErrorLike) => {
        if (!active) return
        setData(null)
      ...
      <div className="dashboard-analytics-history-head rq-dashboard-performance-head">
        ...
        <div className="rq-dashboard-selectors">
          <label className="dashboard-analytics-run-select">
            ...
            <select value={processingId} onChange={(event: any) => setProcessingId(event.target.value)} disabled={!executions.length || loading}>
              {executions.map((item: AppRecord) => <option key={item.id} value={item.id}>{kindLabel(item.kind)} · {shortDateTime(item.finished_at || item.created_at)}</option>)}
            </select>
          </label>
          <label className="dashboard-analytics-run-select">
            <span>{tr('Candidate')}</span>
            <select value={candidateId} onChange={(event: any) => setCandidateId(event.target.value)} disabled={!availableCandidates.length || loading}>
              {availableCandidates.map((item: AppRecord) => <option key={item.candidate_id} value={item.candidate_id}>{item.candidate_id}</option>)}
            </select>
          </label>
        </div>
      </div>
        ...
        <article className="rq-dashboard-heatmap-card">
          <div className="rq-dashboard-heatmap-head">
            ...
            <div className="rotation-monthly-heatmap-modes" role="group" aria-label={tr('Monthly return view')}>
              {[['simulation', 'Strategy'], ['reference', 'Control'], ['excess', 'S − C']].map(([key, label]: any[]) => <button key={key} type="button" className={mode === key ? 'active' : ''} onClick={() => setMode(key)}>{tr(label)}</button>)}
            </div>
          </div>
