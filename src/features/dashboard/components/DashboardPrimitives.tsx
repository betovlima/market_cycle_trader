...
function nextWholeHourTimestamp(now: any = new Date()) {
  const next = new Date(now)
  next.setMinutes(60, 0, 0)
  return next.getTime()
}

function secondsUntil(timestamp: any) {
  return Math.max(0, Math.ceil((timestamp - Date.now()) / 1000))
}

function countdownLabel(totalSeconds: any) {
  const minutes = Math.floor(totalSeconds / 60)
  const seconds = totalSeconds % 60
  return [minutes, seconds].map((value: any) => String(value).padStart(2, '0')).join(':')
}

function nextUpdateLabel(timestamp: any) {
  return new Intl.DateTimeFormat(undefined, { hour: '2-digit', minute: '2-digit' }).format(new Date(timestamp))
}

export function DashboardMetric({ id, label, value, note, tone = '', hint }: AppRecord) {
  return (
...
export function StatusBadge({ status }: AppRecord) {
  return <span className={`table-status ${status || 'unknown'}`}>{tr(String(status || 'unknown').replaceAll('_', ' ').replace(/\b\w/g, (letter: any) => letter.toUpperCase()))}</span>
}

export function DashboardSortHeader({ label, sortKey, sort, onSort, hint }: AppRecord) {
...
export function DashboardPagination({ page, pages, total, onPageChange }: AppRecord) {
...
export function StoryTooltip({ active, payload }: AppRecord) {
  if (!active || !payload?.length) return null
  const point = payload.find((item: AppRecord) => item?.payload?.timestamp_value !== undefined)?.payload
  ...
  return (
    <div className="dashboard-story-tooltip">
      ...
      {events.length ? <div className="dashboard-story-tooltip-events">
        {events.slice(0, 6).map((event: any) => event.grouped ? (
...
export function StoryTradeDot({ cx, cy, payload }: AppRecord) {
  ...
  const grouped = events.find((event: any) => event.grouped)
  if (grouped) {
...
export function SelectedTradeTooltip({ active, payload, selectedTrade, viewMode }: AppRecord) {
  if (!active || !payload?.length) return null
...
export function StrategyForecastTooltip({ active, payload }: AppRecord) {
  if (!active || !payload?.length) return null
...
export function DecisionTimelineTooltip({ active, payload }: AppRecord) {
  if (!active || !payload?.length) return null
  const row = payload[0]?.payload
  if (!row) return null
...
export function TuningTooltip({ active, payload }: AppRecord) {
  if (!active || !payload?.length) return null
  const row = payload[0]?.payload
  if (!row) return null
  return <div className="dashboard-story-tooltip dashboard-intelligence-tooltip">
...
export function StrategyConfigurationGrid({ configuration, modelConfiguration }: AppRecord) {
  const rows = Object.entries(configuration || {}).sort(([left]: any[], [right]: any[]) => left.localeCompare(right))
  const modelRows = Object.entries(modelConfiguration || {}).sort(([left]: any[], [right]: any[]) => left.localeCompare(right))
  ...
  return <details className="dashboard-strategy-config">
    ...
    <div className="dashboard-strategy-config-body">
      {rows.length ? <section><h4>{tr('Strategy')}</h4><dl>{rows.map(([key, value]: any[]) => <div key={key}><dt>{key}</dt><dd>{strategyValue(value)}</dd></div>)}</dl></section> : null}
      {modelRows.length ? <section><h4>{tr('Research Model')}</h4><dl>{modelRows.map(([key, value]: any[]) => <div key={key}><dt>{key}</dt><dd>{strategyValue(value)}</dd></div>)}</dl></section> : null}
    </div>
  </details>
}
