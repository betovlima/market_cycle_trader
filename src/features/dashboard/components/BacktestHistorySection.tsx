...
<section className="dashboard-history-section">
          ...
          <div className="dashboard-history-toolbar">
            <label className="dashboard-list-search"><SearchIcon size={15} /><input type="search" value={query} onChange={(event: any) => onQueryChange(event.target.value)} placeholder={tr("Filter execution history")} aria-label={tr("Filter execution history")} /></label>
            <div className="dashboard-status-filters" role="group" aria-label={tr("Backtest status filter")}>
              {[['all', 'All'], ['completed', 'Completed'], ['interrupted', 'Interrupted'], ['failed', 'Failed'], ['active', 'Active']].map(([value, label]: any[]) => <button key={value} type="button" className={statusFilter === value ? 'active' : ''} onClick={() => onStatusFilterChange(value)}>{tr(label)}<span>{statusCounts[value]}</span></button>)}
            </div>
          </div>
          <div className="table-wrap dashboard-history-table-wrap">
            <table className="dashboard-table dashboard-sortable-table">
              ...
              <tbody>{visibleRows.length ? visibleRows.map((item: AppRecord) => <tr key={item.id}>
