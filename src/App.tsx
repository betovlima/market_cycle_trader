...
function AuthenticatedApp({ session, onLogout, onSessionExpired, onSessionRefresh }: AppRecord) {
  const workspace = useBacktestWorkspace()
  const [activeTab, setActiveTab] = useState('dashboard')
  const [dashboardProcessingId, setDashboardProcessingId] = useState('')
  const capabilities = session?.capabilities || {}
  const allowedTabs = useMemo(
    () => Object.entries(TAB_CAPABILITIES).filter(([, capability]: any[]) => hasCapability(capabilities, capability)).map(([tab]: any[]) => tab),
    [capabilities],
  )
  ...
  useEffect(() => {
    const openDashboardProcessing = (event: any) => {
...
export default function App() {
  ...
  useEffect(() => {
    let active = true
    apiFetch(`${API}/auth/session`).then((value: any) => {
      if (!active) return
      if (value?.authenticated) { setSession(value); setState('authenticated') }
      else { setState('anonymous') }
    }).catch((e: any) => { if (active) { setError(e.message); setState('anonymous') } })
    return () => { active = false }
  }, [])
  ...
  const authenticated = useCallback((value: any) => {
    setError('')
    setSession(value)
