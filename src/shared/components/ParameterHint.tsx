...
export function ParameterHint({
  ...
  const normalizedDetails = Array.isArray(details) ? details.filter((item: AppRecord) => item && (item.label || item.value)) : []
    ...
    >
      ...
      {normalizedDetails.length ? (
        <span className="parameter-hint-detail-list">
          {normalizedDetails.map((item: AppRecord, index: number) => (
